...
if [[ ! "${test_bucket}" =~ ^[a-z0-9][a-z0-9.-]{1,61}[a-z0-9]$ ]]; then
  echo "MINIO_TEST_BUCKET contains unsupported characters" >&2
  exit 1
fi

if [[ ! "${test_bucket}" =~ (^|[.-])test([.-]|$) ]]; then
  echo "MINIO_TEST_BUCKET must include a standalone test segment (for example ai-order-test-private)" >&2
  exit 1
fi
...
echo "Dedicated test database and bucket are ready: ${test_database}, ${test_bucket}"
