...
async function loadProducts() {
  const catalog = await request<Row>('/catalog/configuration/active')
  activeCatalogVersionId.value = Number(catalog.config_version_id) || null
  const materialsByProduct = new Map<number, string[]>()
  for (const material of catalog.materials ?? []) {
    const productId = Number(material.product_id)
    const label = [material.brand_name, material.display_name, material.specification].filter(Boolean).join(' · ')
    const values = materialsByProduct.get(productId) ?? []
    if (label && !values.includes(label)) values.push(label)
    materialsByProduct.set(productId, values)
  }
  products.value = (catalog.products ?? []).map((item: Row) => ({
    ...item,
    product_name: item.display_name,
    product_type: item.workflow_product_type || item.product_code,
    material_spec: businessProductMaterial((materialsByProduct.get(Number(item.product_id)) ?? []).join('；')),
    status: 'ACTIVE',
    price_note: item.pricing_status === 'PRICED' ? '已按当前发布目录配置' : '价格待配置',
    updated_at: null
  }))
}
...
<template>
  <section class="arp-page" :data-route="activeRoute">
    ...
    <template v-else-if="activeRoute === '/system/form-configs'">
      <div class="arp-table-card arp-fill-card"><div class="arp-toolbar"><label class="arp-search"><span>⌕</span><input v-model="keyword" placeholder="搜索产品名称、类型或材料"></label><select v-model="statusFilter"><option value="ALL">全部状态</option><option value="ACTIVE">启用</option></select><button @click="clearFilters">清空</button><button @click="refresh">刷新</button><em>当前发布目录共 {{ filteredProducts.length }} 个产品</em></div><div class="arp-table-scroll"><table class="arp-wide"><thead><tr><th>产品名称</th><th>产品类型</th><th>材料规格</th><th>基础价格</th><th>币种</th><th>启用状态</th><th>价格备注</th><th>目录版本</th><th>操作</th></tr></thead><tbody><tr v-for="row in pagedRows" :key="row.product_id" @click="openDrawer('产品详情', 'product', row)"><td><span class="arp-product-icon">🦷</span><strong>{{ row.product_name }}</strong><small>产品编号 {{ row.product_id }}</small></td><td>{{ productLabel(row.product_type) }}</td><td>{{ row.material_spec || '暂未维护' }}</td><td>{{ row.base_price_cents == null || row.base_price_cents <= 1 ? '价格待配置' : amount(row.base_price_cents, row.currency) }}</td><td>{{ row.currency }}</td><td><i class="arp-badge ok">启用</i></td><td>{{ row.price_note }}</td><td>{{ activeCatalogVersionId ? `#${activeCatalogVersionId}` : '未发布' }}</td><td><button @click.stop="openDrawer('产品详情', 'product', row)">查看</button></td></tr></tbody></table></div><footer class="arp-pagination"><span>用于核对医生当前可下单产品；新增或修改请进入“下单内容设置”</span><div><button :disabled="page <= 1" @click="page--">上一页</button><b>{{ page }}</b><button :disabled="page >= pageCount" @click="page++">下一页</button></div></footer></div>
    </template>
