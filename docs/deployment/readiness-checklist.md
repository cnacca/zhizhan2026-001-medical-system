...
| AI 接入 | PARTIAL | 已完成 DeepSeek 适配、每用户小时限流、单次成本审计、AI 模型重试第一增量、AI 模型失败审计第一增量、AI 治理摘要第一增量、AI 预算阈值第一增量和 AI 预算超限审计第一增量；`AI_MODEL_MAX_RETRIES` 默认 1 次，真实模型短暂 5xx 或连接异常可重试，重试耗尽后写 `AI_MODEL_FAILED` 并返回 503，内部端可通过 `/ai/governance/summary` 查看近 24 小时成功、限流、模型失败、估算成本、`budget_exceeded`、`budget_alert_count` 和 `latest_budget_alert_at`。仍需真实 key 环境联调、预算通知推送、分角色/分模型预算、熔断/降级告警、成本趋势、提示词版本和输出防护；复测 AI-3 越权。 |
...
- 2026-07-02：返工影响审计可视化第一增量已落地；正式上线仍需返工影响图形化、绩效完整公式/周期、生产级 AI 治理和部署交付材料。

- 2026-07-02：返工影响筛选第一增量已落地；正式上线仍需返工影响图形化、绩效完整公式/周期、生产级 AI 治理和部署交付材料。

- 2026-07-02：绩效明细第一增量已落地；正式上线仍需绩效周期筛选、完整公式、标准工时配置、申诉闭环、生产级 AI 治理和部署交付材料。

- 2026-07-02：AI 调用限流第一增量已落地；正式上线仍需成本统计、提示词版本、重试/熔断、输出防护、告警、真实 key 环境联调和部署交付材料。

- 2026-07-02：AI 成本审计第一增量已落地；正式上线仍需成本汇总、预算告警、提示词版本、重试/熔断、输出防护、真实 key 环境联调和部署交付材料。

- 2026-07-02：AI 模型重试第一增量已落地；正式上线仍需失败审计、熔断/降级告警、提示词版本、输出防护、预算告警、真实 key 环境联调和部署交付材料。
...
- 2026-07-02：AI 预算超限审计第一增量已落地；正式上线仍需预算通知推送、分角色/分模型预算、熔断/降级、提示词版本、输出防护、真实 key 环境联调和部署交付材料。
