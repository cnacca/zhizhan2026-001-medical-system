...
更新日期：2026-08-25（顶部校准；下方保留历史追加记录）

## 2026-08-25 当前覆盖口径

正式版本 `7013b1434759df6ca77a65893dbdcf0129e9af7b` 的自动发布 #32742027332 已成功完成
后端 358 项、release gates、双镜像校验、完整发布前 MySQL 备份、双容器替换、健康检查和公网探针；
正式域名 HTTPS、医生端英文和 D-202 站内文件预览／STL 3D 查看均已有上线证据。

因此，下方“真实服务器未部署、HTTPS 未完成、数据库备份完全没有”等旧句只作历史记录，不再代表当前事实。
仍未关闭的是数据库真实恢复、MinIO 对象备份／恢复、日志轮转、监控告警、真实回滚、真实 AI、
真实弱网／跨设备、双实例 WebSocket、客户输入、培训和最终总体验收。当前权威缺口见
`docs/DELIVERY-GAP.md`；Task 8 继续保持 `NOT_READY`。

## 2026-07-15 当前覆盖口径

GOAL-023 / TASK-024 已关闭本地 PRD 余项：当前38项为30 PASS、1 PARTIAL、0 MISSING、7 EXTERNAL_ACCEPTANCE，隔离种植 STL 主链 smoke 已通过。该结果不改变上线边界：CP-004 标准工时、真实模型/对象存储/弱网、HTTPS/部署和最终总体验收仍未完成，Task 8 保持 `NOT_READY`。
...
- 正式环境必须外部注入以下变量：数据库账号密码、Redis、MinIO access/secret、浏览器可达的 `MINIO_PUBLIC_ENDPOINT`、DeepSeek API Key、`APP_AUTH_TOKEN_SECRET` 或正式 JWT 密钥、`APP_AUTH_REFRESH_TOKEN_TTL_SECONDS`、AI 外部 webhook URL、AI 外部 webhook signing secret、HTTPS 证书路径或托管配置。
- `APP_CORS_ALLOWED_ORIGIN` 必须等于用户实际访问前端的完整 origin；8088 联调复测见 `docs/deployment/8088-redeployment-checklist-20260811.md`。
