...
| AI 接入 | PARTIAL | 已完成 DeepSeek 适配、每用户小时限流、单次成本审计和 AI 模型重试第一增量；`AI_MODEL_MAX_RETRIES` 默认 1 次，真实模型短暂 5xx 或连接异常可重试。仍需真实 key 环境联调、失败审计、熔断/降级告警、成本汇总/预算告警、提示词版本和输出防护；复测 AI-3 越权。 |
