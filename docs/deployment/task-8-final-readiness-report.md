...
| 前端业务页面完整度 | readiness checklist 中“前端业务页面”为 PARTIAL；已具备四入口登录、医生下单、客服初审、生产审核、工序实例、质检工时、绩效、生产看板、返工和终检报告第一增量。 | 仍缺实时自动保存、完整客服协同、完整账单物流、设计稿预览 URL 聚合、返工影响图形化、终检专用角色/附件、绩效周期筛选/公式/申诉等完整页面闭环。 | 先做一个可验收页面闭环，例如终检专用角色/附件第一增量，或绩效周期筛选第一增量。 | `npm run build:frontend`；对应 smoke；后端目标测试；客户按 12 步主链路点击验收。 |
...
| 返工 / 绩效 / 终检业务完整度 | acceptance matrix 中返工流程、绩效统计、终检发货仍为 PARTIAL。 | 已有返工关闭、责任分类、影响范围、绩效归因、绩效明细、终检报告第一增量，但缺返工影响图形化、字典后台维护、标准工时、绩效完整公式/周期/申诉、终检附件/PDF/签名和终检专用角色。 | 先做绩效周期筛选第一增量或终检附件/角色第一增量，避免一次扩展过大。 | `CheckWorklogPerformanceTests`；前端 smoke；OpenAPI 检查；客户验收公式/字段。 |
...
下一轮唯一推荐目标：AI 外部告警接收端 webhook 验签/防重放第一增量。
