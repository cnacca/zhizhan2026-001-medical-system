...
| 前端业务页面完整度 | readiness checklist 中“前端业务页面”为 PARTIAL；已具备四入口登录、医生下单、客服初审、客服协同台消息审核、客服资料缺失提示、AI 翻译草稿确认、设计稿预览 URL 聚合、账单物流预览/录入、生产审核、工序实例、质检工时、绩效、生产看板、返工、返工影响图形化、终检报告、终检专用角色 / 附件第一增量和 9D.62 12 步主链路浏览器 smoke 第一增量。 | 9D.62 当前只固定入口可达 smoke；仍缺实时自动保存、客服端设计稿审核预览增强、付款状态、真实物流平台、绩效周期筛选/公式/申诉等完整页面闭环。 | 下一步增强 `npm run smoke:task9d62`，固定最小演示数据并把医生下单到确认收货跑成可复验证据。 | `npm run check:task9d62`；`npm run smoke:task9d62`；`npm run build:frontend`；后端目标测试；客户按 12 步主链路点击验收。 |
...
| 订单主链路完整端到端 | acceptance matrix 中医生下单、客服审核、生产审核、设计稿、账单物流、确认收货多项仍为 PARTIAL；9D.62 已补 12 步入口 smoke。 | 已有大量后端和页面第一增量，但缺完整 12 步端到端数据动作、实时自动保存、完整客服协同、账单/物流平台、付款状态和设计稿多轮回归。 | 增强 12 步主链路 e2e smoke，固定最小演示数据和业务动作。 | `npm run smoke:task9d62`；`npm run build:frontend`；`platform-server test`；客户按矩阵逐项签字。 |
