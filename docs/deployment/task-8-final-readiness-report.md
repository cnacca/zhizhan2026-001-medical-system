...
| 前端业务页面完整度 | readiness checklist 中“前端业务页面”为 PARTIAL；已具备四入口登录、医生下单、客服初审、生产审核、工序实例、质检工时、绩效、生产看板、返工、终检报告和终检专用角色 / 附件第一增量。 | 仍缺实时自动保存、完整客服协同、完整账单物流、设计稿预览 URL 聚合、返工影响图形化、绩效周期筛选/公式/申诉等完整页面闭环。 | 先做返工影响图形化第一增量，把已存在的返工影响节点审计字段转成可读工序影响图。 | `npm run build:frontend`；对应 smoke；后端目标测试；客户按 12 步主链路点击验收。 |
...
| 返工 / 绩效 / 终检业务完整度 | acceptance matrix 中返工流程、绩效统计、终检发货仍为 PARTIAL；9D.55 已补返工字典后台维护第一增量，9D.56 已补终检专用角色 / 附件第一增量。 | 已有返工关闭、责任分类、字典后台维护、影响范围、绩效归因、绩效明细、终检报告和内部附件绑定第一增量，但缺返工影响图形化、标准工时、绩效完整公式/周期/申诉、终检 PDF/签名和真实物流平台。 | 先做返工影响图形化第一增量，避免一次扩展到绩效完整公式、PDF 签章或真实物流。 | `CheckWorklogPerformanceTests`；前端 smoke；OpenAPI 检查；客户验收公式/字段。 |
