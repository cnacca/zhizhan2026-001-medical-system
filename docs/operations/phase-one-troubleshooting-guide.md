...
2026-07-07 操作手册 / 回滚 / 培训材料本地收口已新增 `docs/operations/phase-one-rollback-runbook.md` 作为本地模板；该模板不代表真实发布回滚演练完成。

发布前停机沟通：

1. 确认发布窗口、影响范围、通知对象和回滚负责人。
2. 确认医生端下单、客服初审、生产工序和物流确认是否需要暂停。
3. 记录停机公告和恢复公告位置；真实公告内容保持在外部交付系统，不写入仓库。

备份恢复演练：

1. 发布前确认数据库备份、对象存储快照和恢复负责人。
2. 当前仓库只保留 `待填写` / `待确认` 模板，不记录真实备份路径或凭据。
3. 未完成真实备份恢复演练时，不得把部署 readiness 写成 READY。

回滚触发条件：

- 医生端出现内部字段泄露。
- P0 主链路登录、下单、客服审核、生产审核、入检 / 出检、终检、物流或确认收货中断。
- 正式环境鉴权门禁被放宽。
- 数据库迁移、对象存储、Redis、WebSocket、AI webhook 或 Nginx HTTPS 影响主链路。
