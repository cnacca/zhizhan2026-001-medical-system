...
> 管理端实施补充：本页记录的是三端参考研究基线；管理端菜单、职责、页面合并和工作树规则若与 `docs/design/admin-portal/IMPLEMENTATION_BASELINE.md`、`PAGE_COMPOSITION_MAPPING.md` 或用户最新确认冲突，以后者为准。
...
设备管理、物料管理、安环管理和成本管控保留为独立入口。物料管理当前严格限定为物料异常管理；外协管理必须是独立路由和独立页面，不能与成本管控共用页面。
...
| 工艺生产 | 单页以订单生产进度为主框架；派工只作为具体工序节点执行信息；不提供固定工艺链独立入口 |
| 产品总览 | 单页只读产品列表与 560px 详情；不提供产品或下单字段配置 |
| 质量管理 | 单页质量监督与最终关闭；“返工原因与责任”从标题区进入设置 |
| 通知中心 | 当前账号真实业务通知、全部/未读与已读操作；不做审计中心 |
| 智能服务 | 单页全平台运行、风险、费用与预算汇总；不展示助手目录或技术配置 |
...
2. 工作树与分支规则服从当前任务明确要求；管理端批量重构在用户指定的同一主工作树连续执行，不创建额外 `worktree`，并保留既有未提交改动。
