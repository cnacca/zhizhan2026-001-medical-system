...
| 客服沟通中心 | 三方沟通中转、待审核消息队列、AI 客服查询。 | PARTIAL | 客服协同台第一增量可审核消息并看订单上下文；9D.88 已补生产端消息待审核、客服驳回、医生隐藏和生产端驳回通知 smoke；9D.92 已补客服端 `/ai/cs` AI-2 查询助手入口。 | 仍需补消息附件、完整客服知识上下文聚合、真实浏览器可见范围矩阵和客户 / PM 确认。 |
...
| 客服配送管理 | 人工录入承运商和运单号；真实物流 API 二期。 | PARTIAL | 物流录入和终检发货门禁已有第一段；9D.91 已新增客服端 `/delivery` 配送管理页、`/logistics/orders` 配送列表和 `/orders/{orderId}/logistics/exception` 人工异常跟进，内部跟进说明不返回医生端。 | 仍缺真实物流 API、电子面单、自动轨迹同步、签收回调、物流平台 webhook、物流字段客户 / PM 最终确认和真实浏览器完整验收；不接真实 DHL / FedEx / 顺丰 API。 |
...
| 专项质量管理 | 内返、外返、质量统计看板，内返率 / 外返率分开。 | PARTIAL | 返工、内返率 / 外返率只读汇总第一增量已有；9D.87 已复用 `check_record` + `rework_record`，新增 `/quality-records`、`/quality-records/external-returns` 和生产端 `/production/quality` 外返登记 / 列表入口。 | 仍缺独立 `quality_record` 事实表或客户确认的最终质量记录模型、完整状态处理、编辑/删除、质量复盘、投诉/退货系统和客户 / PM 质量口径确认。 |
| 绩效统计 | 完成数量、有效工时、返工次数、准时率、通过率、工时效率。 | PARTIAL | 绩效周期、公式版本、标准工时覆盖率、默认绩效分已有第一段。 | 标准工时与公式仍需客户 / PM 确认；申诉、导出、工资结算不纳入一期必做。 |
| 设备 / 物料 / 安环 / 成本 / 奖惩 | 设备管理、物料异常、安环、成本、奖惩属于一期开发功能。 | PARTIAL | 当前已有 9D.50-9D.54 只读汇总第一增量；9D.95 已新增 `docs/acceptance/phase-one-production-support-closure-plan.md`；9D.95.1 已补设备台账 / 设备事件录入第一增量；9D.95.2 已补物料异常登记 / 处理状态第一增量；9D.95.3 已补安环巡检 / 隐患整改第一增量；9D.95.4 已补成本记录维护 / 趋势口径第一增量；9D.95.5 已补奖惩记录 / 审批状态第一增量。 | 仍缺设备编辑 / 事件状态更新、物料编辑 / 处理历史、安环复查 / 完整审批、成本编辑 / 审批、奖惩编辑 / 复杂审批、真实趋势、客户 / PM 验收口径和完整页面闭环。 |
...
| AI-2 客服查询助手 | 客服读内部数据，自然语言查询，不自动对外发送。 | PARTIAL | AI-2 后端能力和权限边界已有第一增量；9D.92 AI-2 客服查询助手完整入口第一增量已补客服端 `/ai/cs` 完整入口，调用 `/ai/cs-query` 并展示人工复核提示；9D.94 已补显式 LangChain + DeepSeek provider；9D.97 已补 AI-2 客服查询引用数据说明 / 知识上下文补强第一增量，`/ai/cs-query` 返回 `reference_data_notes`，客服端展示“引用数据说明”。 | 仍需补更完整客服知识上下文、消息附件预览聚合、真实 key 验收和客户 / PM 口径确认。 |
...
| 2 | 生产支撑模块编辑 / 历史补强第一段 | 设备、物料、安环、成本、奖惩已完成最小创建 / 状态第一段，但仍缺编辑、历史和更完整页面闭环。 | 目标后端测试、OpenAPI、前端 build |
| 3 | 质量记录独立模型 / 状态工作流第一段 | 质量记录仍复用 `check_record` + `rework_record`，完整独立模型和客户最终质量口径未关闭。 | 目标后端测试、OpenAPI、前端 build |

## 9D.93.1 PRD V2 范围纠偏记录

- 医生文件模块不属于项目需求范围，不开发；已移除医生端独立 `/doctor/files` / `doctor-files` 入口，订单附件、病例资料、口扫、图片和处方继续归入医生订单链路。
- 设备 / 物料 / 安环 / 成本 / 奖惩属于一期开发功能；当前 9D.50-9D.54 只读汇总已补到最小创建 / 状态第一段，但仍需继续拆编辑、历史、完整审批 / 处理状态、真实趋势和验收闭环。
...
- 新增 `docs/acceptance/phase-one-production-support-closure-plan.md`，把设备管理、物料异常、安环管理、成本管理、奖惩管理拆成后续五个本地可关闭小闭环。
- 当前 9D.50-9D.54 只读汇总只是第一增量，不能写成 READY。
- 推荐实现顺序为 9D.95.1 设备台账 / 设备事件录入、9D.95.2 物料异常登记 / 处理状态、9D.95.3 安环巡检 / 隐患整改、9D.95.4 成本记录维护 / 趋势口径、9D.95.5 奖惩记录 / 审批状态。
- 本轮不接 IoT，不接真实财务系统，不把奖惩作为工资发放结果，不新增数据库迁移或业务接口。
...
当前项目不是“离正式上线只差签收模板”，而是需要继续按 PRD V2.0 关闭一期缺口。Task 8 仍保持 `in-progress / NOT_READY`。9D.83 已完成患者管理基础版第一增量，9D.84 已完成人工支付流水 / 收支记录第一增量，9D.85 已完成客户 / 诊所档案与偏好第一增量，9D.86 已完成人员档案 / 工作量看板第一增量，9D.87 已完成质量记录 CRUD / 外返登记第一增量，9D.88 已完成客服订单 / 沟通完整可见性 smoke，9D.89 已完成医生账户设置基础闭环，9D.90 已完成产品参数 / 价格体系一期最小后台，9D.91 已完成客服配送管理页 / 物流异常跟进第一增量，9D.92 已完成 AI-2 客服查询助手完整入口第一增量，9D.93.1 已按用户确认完成 PRD V2 范围纠偏，9D.94 已完成 LangChain + DeepSeek AI 底座对齐第一增量，9D.95 已完成设备 / 物料 / 安环 / 成本 / 奖惩一期闭环拆解第一增量，9D.95.1 已完成设备台账 / 设备事件录入第一增量，9D.95.2 已完成物料异常登记 / 处理状态第一增量，9D.95.3 已完成安环巡检 / 隐患整改第一增量，9D.95.4 已完成成本记录维护 / 趋势口径第一增量，9D.95.5 已完成奖惩记录 / 审批状态第一增量，9D.96 已完成医生提交前 AI-4 资料缺失自动触发体验第一增量，9D.97 已补 AI-2 客服查询引用数据说明 / 知识上下文补强第一增量，9D.98 已补 AI-5 生产备注客户模板 / 知识上下文补强第一增量；下一步推荐真实客户 / PM 确认项与真实环境 AI 验收收口。
