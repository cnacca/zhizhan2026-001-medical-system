...
| 顺序 | 最小闭环 | 原因 | 验收命令建议 |
| --- | --- | --- | --- |
| 1 | 客户 / 诊所档案与偏好第一增量 | PRD 客服客户管理和医生下单偏好自动填充都依赖该基础。 | 后端目标测试、OpenAPI、前端 build |
| 2 | 人员档案 / 工作量看板第一增量 | PRD 生产端人员管理一期必做；可复用现有用户、部门、岗位和绩效事实。 | 后端目标测试、前端 build、acceptance |
| 3 | 质量记录 CRUD / 外返登记第一增量 | PRD 专项质量管理一期必做，当前只有返工和汇总。 | 后端目标测试、OpenAPI、前端 build |
...
当前项目不是“离正式上线只差签收模板”，而是需要先按 PRD V2.0 重排一期缺口。Task 8 仍保持 `in-progress / NOT_READY`。9D.83 已完成患者管理基础版第一增量，9D.84 已完成人工支付流水 / 收支记录第一增量；下一步推荐“客户 / 诊所档案与偏好第一增量”。
