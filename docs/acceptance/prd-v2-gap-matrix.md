...
| 医生患者管理 | 一期基础患者档案、绑定历史病例、患者列表检索。 | PARTIAL | 9D.83 基础版之上已补出生日期、联系方式、病史/过敏、标签、治疗状态、疗程日期、最近产品、患者详情/更新接口，以及参考式列表、新增弹窗和可编辑详情抽屉；订单绑定、历史订单、历史参考、患者下单和本人数据隔离继续保留。 | 仍缺真实客户患者数据导入、跨诊所批量检索、AI 历史方案推荐和客户 / PM 对患者字段最终口径确认。 |
