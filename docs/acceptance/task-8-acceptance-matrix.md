...
GOAL-021 / TASK-022 已把 PRD V2 的31项主验收和7项设计稿验收重新整理为统一38项矩阵：`docs/acceptance/prd-v2-38-item-acceptance-audit-20260715.md`。当前结果为18 PASS、8 PARTIAL、4 MISSING、8 EXTERNAL_ACCEPTANCE。旧 CP-001 到 CP-009 不再整体作为书面确认阻塞；只有动态表单字段和文件限制2项待产品确认，PRD逐功能签字为0项。下文旧阶段记录继续保留历史证据，如状态冲突，以本节和38项矩阵为准。Task 8 仍保持 `NOT_READY`。

2026-07-07 部署 / 运维本地补强由 `GOAL-020` / `TASK-021` 承接，并新增 `npm run check:deployment-ops-local-hardening` 与 `npm run dry-run:phase-one-release-rollback`。本阶段只补本地 release / rollback dry-run、备份 / 恢复 dry-run 模板第一段、日志留存 / 监控告警配置模板第一段、compose / env / Nginx / healthcheck 静态检查和 readiness 联动；不填写真实服务器地址，不填写真实密钥，不代表真实服务器部署、HTTPS、备份恢复、日志留存、监控告警、发布回滚演练、正式客户培训签收、客户 / PM 签字或真实环境验收完成，Task 8 仍保持 `NOT_READY`。

2026-07-07 AI 生产治理本地补强由 `GOAL-019` / `TASK-020` 承接，并新增 `npm run check:ai-production-governance-local-hardening`。本阶段只补本地只读治理总览、提示词版本、输出安全边界、预算 / 熔断、AI-3 安全矩阵、AI-5 默认模板边界和真实外部集成待验状态；不填写真实 DeepSeek key，不填写真实 webhook URL，不确认客户正式 AI-5 模板，不代表客户 / PM 签字或真实环境验收完成，Task 8 仍保持 `NOT_READY`。

2026-07-07 本地 12 步主链路验收增强由 `goals/GOAL-018-local-main-chain-acceptance-hardening-20260707.md` / `tasks/TASK-019-local-main-chain-acceptance-hardening-20260707.md` 承接，并新增 `npm run check:local-main-chain-acceptance-hardening`。本阶段只增强本地固定演示数据 smoke 和客户可读验收记录：医生端脱敏、客服端可见性、生产端任务范围、管理端派工 / 转派角色边界断言；不代表客户 / PM 签字、真实支付 / 物流平台、真实 DeepSeek key、真实 webhook、真实电子签章或真实环境验收完成，Task 8 仍保持 `NOT_READY`。

2026-07-07 四端前端产品化体验收口由 `goals/GOAL-017-frontend-productization-closure-20260707.md` / `tasks/TASK-018-frontend-productization-closure-20260707.md` 承接，并新增 `npm run check:frontend-productization-closure`。本阶段只补本地可开发的前端体验：医生端继续保持订单中心八大模块和安全外部视图，客服设计稿 / 账单入口复用已有本地链路，生产 C 类入口升级为本地第一增量汇总 / 表单，管理端补账号 / 角色 / 权限库存入口和统一加载态 / 空态 / 错误态 / 权限拒绝态提示。`frontend-business-pages` 仍为 `PARTIAL`，不恢复医生文件独立模块，不扩大 C 类为完整闭环，客户 / PM 签字、真实支付 / 物流平台、真实 DeepSeek key、真实 webhook、真实电子签章和真实环境验收仍未完成，Task 8 仍保持 `NOT_READY`。

2026-07-07 权限 / DataScope 生产化补强 B 由 `goals/GOAL-016-auth-datascope-production-closure-b-20260707.md` / `tasks/TASK-017-auth-datascope-production-closure-b-20260707.md` 承接，并新增 `npm run check:auth-datascope-prod-closure-b`。本阶段补 refresh token 轮换：`/api/auth/refresh` 成功后返回新 access token 和轮换后的 refresh token，旧 refresh token 立即吊销并拒绝复用；该记录不代表完整 Spring Security/JWT、完整 RuoYi DataScope、通用 SQL DataScope、access token 黑名单、多设备会话策略、客户签字或真实环境验收完成。`auth-datascope-prod` 仍为 `PARTIAL`，Task 8 仍保持 `NOT_READY`。

2026-07-07 操作手册 / 回滚 / 培训材料本地收口由 `goals/GOAL-015-operations-rollback-training-closure-20260707.md` / `tasks/TASK-016-operations-rollback-training-closure-20260707.md` 承接，并新增 `npm run check:operations-rollback-training-closure`。本阶段新增 `docs/operations/phase-one-rollback-runbook.md` 和 `docs/operations/phase-one-training-materials.md`，并回写操作手册、故障处理清单和交付材料索引；这些模板默认 `待填写` / `待确认`，不填写真实服务器地址、真实密钥、真实签名、真实客户名单或客户隐私数据，不代表真实发布回滚演练、备份恢复演练、监控告警、正式客户培训签收或客户 / PM 签字完成。`operations-manuals` 仍为 `PARTIAL`，Task 8 仍保持 `NOT_READY`。

2026-07-07 WebSocket / 通知生产 readiness 收口由 `goals/GOAL-014-websocket-notification-readiness-closure-20260707.md` / `tasks/TASK-015-websocket-notification-readiness-closure-20260707.md` 承接，并新增 `npm run check:websocket-notification-readiness-closure`。本阶段新增 `docs/deployment/websocket-notification-production-readiness.md` 作为真实环境通知验收记录模板；该模板默认 `待填写` / `待确认`，不填写真实密钥、真实 webhook URL、真实主机、证书私钥、token 或客户隐私数据，不代表真实双实例 Redis、Nginx HTTPS、生产 webhook 或真实环境通知验收完成。`websocket-notification-prod` 仍为 `PARTIAL`，Task 8 仍保持 `NOT_READY`。

2026-07-07 四端业务页面与客户验收 smoke 收口由 `goals/GOAL-013-frontend-customer-smoke-closure-20260707.md` / `tasks/TASK-014-frontend-customer-smoke-closure-20260707.md` 承接，并新增 `npm run check:frontend-customer-smoke-closure`。本矩阵继续记录 Task 8 验收状态；`frontend-business-pages` 仍为 `PARTIAL`，客户 / PM 签字、真实支付 / 物流平台、真实 DeepSeek key、真实 webhook、真实电子签章和真实环境验收仍未完成，Task 8 仍保持 `NOT_READY`。

2026-07-07 权限 / DataScope 生产化收口第一段由 `goals/GOAL-012-auth-datascope-production-closure-20260707.md` / `tasks/TASK-013-auth-datascope-production-closure-20260707.md` 承接，并新增 `npm run check:auth-datascope-prod-closure`。本矩阵继续记录 Task 8 验收状态，不代表上线 READY；完整 Spring Security/JWT、完整 RuoYi DataScope、通用 SQL DataScope 拦截器、refresh token 轮换、access token 黑名单、多设备会话策略、客户签字和真实环境验收仍未完成。

2026-07-07 客户 / PM 确认项与真实环境 AI 验收收口由 `goals/GOAL-011-real-acceptance-confirmation-20260707.md` / `tasks/TASK-012-real-acceptance-confirmation-20260707.md` 承接，并新增 `npm run check:real-acceptance-confirmation`。本矩阵继续记录 Task 8 验收状态，不代表上线 READY；真实 key、webhook、客户签字和真实环境验收仍为待填写 / 待确认。

2026-07-07 RepoFrame 文档校准由 `goals/GOAL-003-repoframe-doc-hydration-20260707.md` / `tasks/TASK-004-repoframe-doc-hydration-20260707.md` 承接。本矩阵继续保留该历史锚点。
...
权限 / DataScope 生产化收口第一段已推进为 GOAL-012 / TASK-013，新增 `npm run check:auth-datascope-prod-closure`。本批次补 `StrictPermissionModeTests` 医生账户设置权限码目标测试、`clinic:*` / `account:doctor` / `notification:*` 权限码、V36 权限码种子，并清零主代码 roles-only `@RequirePermission` 注解。权限 / DataScope 生产化补强 B 已推进为 GOAL-016 / TASK-017，新增 `npm run check:auth-datascope-prod-closure-b`，并补 refresh token 轮换。该记录不代表完整 Spring Security/JWT、完整 RuoYi DataScope、通用 SQL DataScope 拦截器、access token 黑名单、多设备会话策略、客户签字或真实环境验收完成，Task 8 仍保持 `NOT_READY`。
...
9D.87 质量记录 CRUD / 外返登记第一增量已复用 `check_record` 和 `rework_record`，新增 `/quality-records`、`/quality-records/external-returns`、生产端 `/production/quality` 外返列表 / 登记入口和 `npm run check:task9d87`；CS / ADMIN 可登记外返质量记录并查看内部质量记录，DOCTOR 拒绝访问。PRD V2 本地功能差异收口 B 已新增 `quality_record` 独立事实表、`/quality-records/{qualityRecordId}/status` 状态更新接口、生产端质量页状态更新表单和 `npm run check:prd-v2-gap-closure-b`；创建外返仍保留 `check_record` / `rework_record` 兼容证据。本轮不做编辑/删除、投诉/退货系统、质量复盘完整流程或客户最终质量口径确认，Task 8 仍保持 `NOT_READY`。
...
9D.99 A/B 类一期范围对齐第一段已新增 `npm run check:task9d99`，并完成前端展示层第一段收口：生产端删除独立“工作单”入口，生产展示“生产中”改为“生产异常”，菜单“物料异常”改为“物料管理”，统一“待问异常”；客服工作台补客服统计基础版，包括翻译待审、账单超期、本月 / 上月对比、订单数量 / 件数和十大客户排名；生产工作台补生产统计基础版，包括生产异常、待问异常、员工异常、部门今日 vs 上月平均、返工率、出货率、完成率和内返 / 外返；账单 / 物流人工状态继续按一期人工维护能力展示。本轮不新增后端接口、不新增迁移、不接真实支付 / 物流平台、不伪造真实经营统计，Task 8 仍保持 `NOT_READY`。

9D.100 A/B 类一期范围对齐第二段已新增 `npm run check:task9d100`，并把客服 / 生产工作台统计从展示口径推进到复用现有本地接口的数据闭环：客服侧复用订单列表、待审消息、通知未读、物流人工状态和质量外返汇总；生产侧复用订单列表、待审消息、人员工作量、质量返工、物流人工状态以及设备 / 物料 / 安环 / 成本 / 奖惩基础汇总。月度趋势、真实账期逾期、真实支付平台、真实物流平台和客户 / PM 最终统计口径仍为 PARTIAL / BLOCKED，Task 8 仍保持 `NOT_READY`。

PRD V2 本地功能差异收口 A 已完成为 GOAL-007 / TASK-008，新增 `npm run check:prd-v2-gap-closure-a`。本批次只校准 9D.100 之后的本地差异队列、acceptance/readiness 指针和机器检查：下一批本地实现队列优先为质量记录独立模型 / 状态工作流第一段，月度趋势 / 客户排名完整口径确认和 AI-2 消息附件预览聚合 / 更完整知识上下文继续保持 PARTIAL / BLOCKED 边界。该记录不代表新增业务实现完成，不代表真实 key、真实 webhook、客户签字或真实环境验收完成，Task 8 仍保持 `NOT_READY`。

PRD V2 本地功能差异收口 B 已推进为 GOAL-008 / TASK-009，新增 `npm run check:prd-v2-gap-closure-b`。本批次关闭质量记录独立模型 / 状态工作流第一段的本地实现：V35 `quality_record`、后端状态更新、目标测试、OpenAPI 和前端状态更新入口均已纳入验收。该记录不代表客户最终质量口径、真实支付 / 物流平台、真实 key、真实 webhook、客户签字或真实环境验收完成，Task 8 仍保持 `NOT_READY`。

PRD V2 本地功能差异收口 C 已推进为 GOAL-009 / TASK-010，新增 `npm run check:prd-v2-gap-closure-c`。本批次关闭 AI-2 消息附件预览聚合 / 更完整知识上下文第一段的本地实现：`/ai/cs-query` 响应新增 `attachment_contexts`，通过既有文件权限校验生成短时效预览 URL，客服端 `/ai/cs` 展示“附件预览上下文”，OpenAPI 新增 `AiAttachmentContext`。该记录不代表真实 key、RAG / tool calling、生产 webhook、客户 AI-2 口径、客户签字或真实环境验收完成，Task 8 仍保持 `NOT_READY`。

PRD V2 本地功能差异收口 D 已推进为 GOAL-010 / TASK-011，新增 `npm run check:prd-v2-gap-closure-d`。本批次关闭月度趋势 / 客户排名聚合接口第一段的本地实现：`/dashboards/phase-one-ab` 返回当前月 / 上月订单与件数、月度差值、Top 客户、生产异常、待问异常、出货率和完成率，客服 / 生产工作台消费该聚合，OpenAPI 新增 `PhaseOneAbDashboardResponse`。该记录不代表真实支付 / 物流平台、客户最终统计口径、客户签字或真实环境验收完成，Task 8 仍保持 `NOT_READY`。
...
- 当前实现证据：`OrderStatusProjectionTests`、`FileAccessTests`、`WorkflowRuntimeTests`、`CheckWorklogPerformanceTests`、`MessageDesignBillNotificationTests`、`AiGatewayTests`、`BearerIdentityTests`、`PermissionInterceptorTests`、`StrictPermissionModeTests`、`NotificationWebSocketTests`、`NotificationRestTests`、`NotificationBroadcastTests`、9D.24 四入口登录页校验、9D.56 终检专用角色 / 附件第一增量、9D.57 返工影响图形化第一增量、9D.58 客服协同闭环第一增量、9D.59 客服资料缺失提示与 AI 翻译草稿确认第一增量、9D.60 设计稿预览 URL 聚合第一增量、9D.61 账单物流预览/录入闭环第一增量、9D.62 12 步主链路浏览器 smoke 第一增量、9D.62.1 固定演示数据闭环第一段、9D.62.2 派工与工序操作数据闭环第一段、9D.62.3 设计稿确认数据闭环第一段、9D.62.4 账单/物流数据闭环第一段、9D.62.5 终检后发货与医生确认收货数据闭环第一段、9D.63 返工异常路径数据闭环第一段、9D.64 客服端设计稿审核预览增强第一段、9D.65 终检 PDF/签名第一段、9D.66 绩效周期筛选第一段、9D.69 部署基础设施第一段、9D.75 正式鉴权与 DataScope 收口第一段、9D.76 WebSocket / 通知生产验收第一段、9D.77 文件上传弱网 / 跨设备验收第一段、9D.78 bucket 隔离验收记录第一段、9D.79 真实环境文件上传人工验收记录模板第一段、9D.80 AI 真实 key / 生产 webhook 联调记录模板第一段、9D.81 部署真实环境 smoke / HTTPS / 备份监控验收记录模板第一段、9D.99 A/B 类一期范围对齐第一段、9D.100 A/B 类一期范围对齐第二段和既有 HTTP/SQL smoke 记录。
...
| 9 | 消息按角色可见；生产端发医生前客服审核。 | PARTIAL | `MessageDesignBillNotificationTests` 覆盖 WORKER 消息待审、CS 审核后医生可见、内部备注不泄露；`NotificationWebSocketTests` 覆盖在线通知推送；`NotificationRestTests` 覆盖通知列表、未读数和已读隔离；`NotificationBroadcastTests` 覆盖 Redis 广播代码路径；前端骨架已有通知中心和 WebSocket 实时刷新入口；9D.1 医生订单工作台可读公开消息并发送给客服；9D.58 已补客服协同台待审核消息、订单消息上下文和通过/驳回入口；PRD V2 本地功能差异收口 C 已补 AI-2 `attachment_contexts` 附件预览上下文第一段。 | AI-2 附件预览上下文第一段已补；仍缺完整消息附件工作流、真实双实例 Redis 联调、生产网关验收和完整客服消息 smoke。 |
...
- 当前只剩动态表单最终字段和文件限制2项待客户 / PM 产品确认；AI-5 模板属于客户资料输入，标准工时属于业务数据且提供方待项目方指定，付款与物流的人工维护方式已确认。9D.72 清单已由 GOAL-021 校正，不再要求 CP-001 到 CP-009 逐项签字。
...
- 9D.83 已补患者管理基础版第一增量，9D.84 已补人工支付流水 / 收支记录第一增量，9D.85 已补客户 / 诊所档案与偏好第一增量，9D.86 已补人员档案 / 工作量看板第一增量，9D.87 已补质量记录 CRUD / 外返登记第一增量，PRD V2 本地功能差异收口 B 已补 `quality_record` 独立事实表和状态工作流第一段，9D.88 已补客服订单 / 沟通完整可见性 smoke，9D.89 已补医生账户设置基础闭环，9D.90 已补产品参数 / 价格体系一期最小后台，9D.91 已补客服配送管理页 / 物流异常跟进第一增量，9D.92 已补 AI-2 客服查询助手完整入口第一增量，9D.93.1 已补 PRD V2 范围纠偏第一闭环，9D.94 已补 LangChain + DeepSeek AI 底座对齐第一增量，9D.95.1-9D.95.5 已作为 C 类基础台账 / 基础登记 / 状态更新证据保留，9D.96 已补医生提交前 AI-4 资料缺失自动触发体验第一增量；医生文件模块不属于项目需求范围，不开发；2026-07-06 基准确认设备 / 物料 / 安环 / 成本 / 奖惩不再作为一期完整闭环继续扩展；所有 AI 智能体使用 LangChain + DeepSeek；质量记录完整编辑/删除、客户最终质量口径、AI-2 知识上下文补强和真实环境验收仍需继续按一期基准关闭。
...
证据：`npm run check:task9d93` 检查医生端独立 `/doctor/files` / `doctor-files` 入口已移除，PRD V2 gap matrix、readiness、final readiness、README、tasks 和 acceptance 已记录医生文件模块不开发。2026-07-06 基准另由 `npm run check:scope-baseline-20260706` 固定：设备 / 物料 / 安环 / 成本 / 奖惩作为 C 类基础能力保留，不继续作为一期完整管理闭环扩展；所有 AI 智能体使用 LangChain + DeepSeek 实现。
...
未完成原因：本记录现在只作为历史拆解和 C 类基础能力索引，不新增接口、不新增迁移、不做 CRUD，不接 IoT、真实财务系统或工资发放；设备 / 物料 / 安环 / 成本 / 奖惩完整录入、编辑、处理历史、真实趋势和完整管理闭环不再列为一期本地必做。Task 8 仍保持 NOT_READY。
...
证据：`/ai/cs-query` 响应新增 `reference_data_notes`，覆盖订单基础、生产上下文、沟通消息、附件、账单和物流只读来源说明；客服端 `/ai/cs` 展示“引用数据说明”；`npm run check:task9d97` 检查后端、前端、OpenAPI、README、STATUS、DECISIONS、tasks、acceptance 和缺口文档回写；`AiGatewayTests#csQueryReturnsReferenceDataNotesForAuditableInternalSources` 覆盖目标响应。随后 PRD V2 本地功能差异收口 C 已补 `attachment_contexts` 附件预览上下文、客服端展示、OpenAPI `AiAttachmentContext` 和 `npm run check:prd-v2-gap-closure-c`。

未完成原因：本轮不新增数据库迁移，不接真实 DeepSeek key，不做 RAG / tool calling，不自动发送消息，不自动写入订单、生产备注或客服沟通记录。AI-2 附件预览上下文第一段已补，但真实 key 环境、RAG / tool calling 如需、更完整生产级输出策略和客户 / PM AI-2 口径确认仍未完成。Task 8 仍保持 NOT_READY；随后已推进 AI-5 生产备注客户模板 / 知识上下文补强第一增量。

## PRD V2 本地功能差异收口 C

状态：completed-first-increment / PARTIAL。

证据：GOAL-009 / TASK-010 已补 `/ai/cs-query` 响应 `attachment_contexts`，聚合当前订单已完成附件的有界元数据、短时效 `preview_url` 和人工复核说明；客服端 `/ai/cs` 展示“附件预览上下文”；OpenAPI 新增 `AiAttachmentContext`；`npm run check:prd-v2-gap-closure-c` 检查 GOAL/TASK、后端、前端、OpenAPI、acceptance 和 readiness 回写；`AiGatewayTests#csQueryReturnsMessageAttachmentPreviewContextsForManualReview` 覆盖 CS 可见、DOCTOR 403、响应不返回 `object_key`。

未完成原因：本轮不接真实 DeepSeek key，不做 RAG / tool calling，不接真实 webhook，不自动发送消息，不自动写入订单或客服沟通记录，不向医生暴露内部附件上下文。AI-2 仍需真实 key 环境、客户 / PM AI-2 口径确认和真实环境验收。Task 8 仍保持 `NOT_READY`。

## PRD V2 本地功能差异收口 D

状态：completed-first-increment / PARTIAL。

证据：GOAL-010 / TASK-011 已补 `/dashboards/phase-one-ab`，聚合本地月度趋势 / 客户排名第一段，返回 `current_month`、`previous_month`、`monthly_order_delta`、`top_customers`、`production_exception_count`、`pending_question_count`、`shipping_rate` 和 `completion_rate`；客服 / 生产工作台消费该聚合；OpenAPI 新增 `PhaseOneAbDashboardResponse`、`PhaseOneAbMonthSummary` 和 `PhaseOneAbCustomerRanking`；`npm run check:prd-v2-gap-closure-d` 检查 GOAL/TASK、后端、前端、OpenAPI、acceptance 和 readiness 回写；`PhaseOneDashboardTests` 覆盖 CS 可见和 DOCTOR 403。
