...
| AI 越权 | PASS | `AiGatewayTests` 覆盖 AI-3 内部问题安全拒绝；9D.28 已补 AI 模型重试第一增量，真实模型首次 500、第二次成功时接口返回成功且只写一条成功审计；9D.29 已补 AI 模型失败审计第一增量，重试耗尽时返回 503 并写 `AI_MODEL_FAILED`；9D.30 已补 AI 治理摘要第一增量，内部端可查看近 24 小时成功、限流、模型失败和估算成本；9D.31 已补 AI 预算阈值第一增量，治理摘要可返回 `budget_exceeded`；9D.32 已补 AI 预算超限审计第一增量，真实模型成功调用跨过预算阈值时写入 `AI_BUDGET_EXCEEDED` 并返回 `budget_alert_count`。 | 真实 key 环境、生产提示词、预算通知推送、分角色/分模型预算、熔断/降级告警和输出防护策略仍需上线前复测。 |
...
- 9D.21 已补绩效归因联动第一增量：`/performance` 可拆分生产责任、非生产责任和未归因返工。
...
- 2026-07-02：9D.32 已补 AI 预算超限审计第一增量，真实模型成本跨过预算阈值时写入 `AI_BUDGET_EXCEEDED` 并在治理摘要返回 `budget_alert_count`；Task 8 仍为 NOT READY。
