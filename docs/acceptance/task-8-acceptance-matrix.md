...
| AI 越权 | PASS | `AiGatewayTests` 覆盖 AI-3 内部问题安全拒绝；9D.28 已补 AI 模型重试第一增量，真实模型首次 500、第二次成功时接口返回成功且只写一条成功审计；9D.29 已补 AI 模型失败审计第一增量，重试耗尽时返回 503 并写 `AI_MODEL_FAILED`。 | 真实 key 环境、生产提示词、失败聚合、熔断/降级告警和输出防护策略仍需上线前复测。 |
