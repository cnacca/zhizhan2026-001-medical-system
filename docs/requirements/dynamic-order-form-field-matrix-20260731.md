...
按 D-208／D-210，医生下单不再要求交付格式、设计标准、设计时效或修改政策；设计要求备注也改为选填。这些字段不得以历史草稿规则继续阻止设计服务下单。
...
STL、PLY、OBJ、DICOM、JPG、PNG、PDF；各产品资料要求按下表执行。2026-08-25 用户确认文件要求
...
`POV2-OPEN-003` 和 `POV2-OPEN-008` 已由 D-208 关闭：设计时效不作为医生下单字段；设计服务使用现有账单状态门禁，医生确认且付款后下载原文件，不进入实体物流。
