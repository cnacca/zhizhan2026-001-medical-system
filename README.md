...
2026-07-04 交接摘要：Task 8 总体仍为 `NOT_READY`；9D.48、9D.48.1、9D.48.2 已完成 AI 外部告警 outbox 监控、列表筛选和失败/死信可见性第一增量；Task 8 readiness 终检报告、部署安全 / 环境变量 readiness 检查和验收矩阵机器可读缺口清单第一增量已完成；9D.55 已按 RuoYi-Vue-Pro / 若依 Pro 的字典、CRUD、菜单、权限码范式补返工字典后台维护第一增量；9D.56 已补 `final-inspection:manage`、`final_inspection_report_file` 和终检报告 `attachment_file_ids`，终检报告生成需专用权限，医生端不能读取报告或内部附件预览 URL。`acceptance.json` 现在维护 `task8_readiness_gaps`、9D.55 和 9D.56 必备文本检查，可通过 `npm run check:task8-readiness-gaps` 查看当前上线缺口。下一轮唯一推荐目标是返工影响图形化第一增量。继续开发前请从 `STATUS.md` 和 `tasks/README.md` 的当前交接摘要开始。
...
正式上线仍缺完整 RuoYi 管理 UI、通用 DataScope SQL 拦截器、真实弱网/跨设备续传、设计稿预览 URL 聚合、返工影响图形化、绩效完整公式/周期/申诉/标准工时配置、终检 PDF/签名/真实物流/管理绩效等完整前端业务页面、WebSocket 生产网关/真实多实例验收、外部告警接收端验签/防重放/生产联调、提示词后台管理、流式输出过滤、真实 key 联调、HTTPS/备份/监控/操作手册等硬条件。
```
...
1. 下一轮唯一推荐目标：返工影响图形化第一增量。
2. 复用现有 `ReworkRecordResponse.impacted_node_*` 字段和生产端返工终检页面，先把受影响后续工序转成可读工序影响图。
3. 不做复杂甘特、拖拽排产、绩效申诉或生产通知大改；每补一个缺口，都要回写 `docs/acceptance/task-8-acceptance-matrix.md` 和 `docs/deployment/readiness-checklist.md` 的状态。
