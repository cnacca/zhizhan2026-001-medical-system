...
2026-08-24 已按 D-201 完成本地修改并获授权进入正式发布流程：客服“产品目录”和管理端“产品总览”只读使用医生端当前已发布下单目录，新增／修改统一由管理端“下单内容设置”维护；当前发布版本显示 95 个启用产品，旧产品表中的单条种子数据不再作为目录。医生端主材料、制作项目、牙色系统和色号改为选填，色号支持输入筛选与自定义值，并修复色号候选层、移除确认层被订单向导遮挡的问题；新订单口扫支持 ZIP／STL／PLY／OBJ 单次多选、自动／手工分类，并以“完整资料包”或“上颌＋下颌＋咬合齐全”作为生产完整性门禁，PDF 不算口扫；可修改提示可定位对应步骤、产品和区域。全新专用测试库后端 357 项、前端生产构建、后端 package、正式发布门禁和专项检查全部通过，隔离浏览器已验证上述三项反馈且控制台无错误。尚未提交、推送或部署，Task 8 保持 `NOT_READY`。
