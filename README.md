...
2026-09-03：D-217 / D-218 和检验任务列表文字重叠修复已发布正式站，版本 `0eb3ffba`（PR #58 / #59，自动部署 #33715356521 成功）。发布包含完整后端 368 项回归、双镜像、数据库备份、服务健康和公网探针；本地 Chrome 10 项通过。详见 `docs/deployment/order-followup-release-20260903.md`。Task 8 仍为 `NOT_READY`。

牙科定制工厂下单与生产协同平台：一期业务基础已形成，当前按二期 M2 / M3 / M6 增量推进；客户只能查看外部简化进度。

## 当前仓库状态

2026-08-25 GOAL-037／TASK-039 本地范围已完成：医生下单四步、设计服务 D-212 三项制作要求以及 D-215 螺旋扩弓“单向／双向”必选规格均已接通；服务端拒绝空规格，订单快照保留方向。D-211 错误推簧别名实现已清除，“推簧”继续 `USER_DEFERRED`。目标测试、专项检查、验收基线和前端生产构建通过；未提交、推送或部署，Task 8 保持 `NOT_READY`。
