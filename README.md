...
2026-08-18 医生端英文界面已按 D-197 完整收口并发布正式站：共享登录页按用户确认保持中文，进入医生端后才应用中文／`EN` 切换。最终正式版本 `f406d105166c0b3bc6c266fead7b80c895b4d470`，自动部署 #32049802365 结果 `success`；后端 351 项测试、release gates、镜像校验、备份、部署、健康检查和公网探针全部通过。线上只读浏览器验证已覆盖 7 个医生端主页、抽屉／对话框、下单向导、动态产品字段和隐形正畸处方；最终版另复验了中文登录页、英文标签页、`A/EN` 消息标记、`YYYY-MM-DD` 日期框和 `Orthodontic Appliance` 产品映射。患者姓名、诊所名、医生输入及历史消息等原始业务数据保留原文；未写入正式业务数据，Task 8 保持 `NOT_READY`。
