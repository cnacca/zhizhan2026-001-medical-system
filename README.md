...
2026-08-15 已关闭正式站登录的 Origin 漂移缺陷：生产运行证据确认 `www`／直接端口页面与仅允许裸域名的后端 CORS 配置不一致。生产环境已兼容裸域名和 `www`；仓库按 D-191 将非规范入口 308 跳转到 `https://chinesedigitaldental.com`，并在每次生产发布后从公网验证两个登录 Origin 和两个重定向入口。该局部生产修复不代表一期整体上线完成，Task 8 保持 `NOT_READY`。
...
四个账号同时作为公开演示站的端口验收账号：`admin / cs / worker / doctor` 分别使用 `ACCEPTANCE_ADMIN_FULL / ACCEPTANCE_CS_FULL / ACCEPTANCE_PRODUCTION_FULL / ACCEPTANCE_DOCTOR_FULL` 聚合所属端能力。前三端数据范围为 `ALL`，医生端为 `CLINIC` 且不包含任何内部生产字段。隔离 `_demo` 数据库由 demo seed 创建这些角色；公开演示站只通过现有 RBAC 管理接口授权并保留审计，不把演示角色写入 Flyway。
