...
2026-08-24 D-202 已完成 2026-08-22 测试报告的一期阻断项、本轮文件预览反馈及回归校准：客服可按系统完整性依据和人工判断明确退回补资料，信息审核页 STL 可直接打开全局 3D 查看器；生产审核员仅在待生产审核状态只读访问订单附件；管理端预览不再通过空白页导航或自动下载，STL、图片／PDF 在站内查看，其他格式明确提示后显式下载；MinIO 预览／下载签名分别使用 `inline`／`attachment`，链接等待保留 15 秒超时。旧 `check:feedback-20260713` 已改为要求站内查看并禁止 `about:blank`／`popup.location.replace`；阻断浏览器 smoke 补普通 `WORKER`＋`SELF` 生产看板路径后为 3/3，已断言 STL canvas、预览／下载请求分离及单页面行为。后端文件权限测试 18/18、前端生产构建、专项／生产看板既有检查通过；真实客服、管理端和管理端生产复核页 STL 已完成点击渲染，真实普通生产账号因无分配工单仍为 `PARTIAL`，未写正式数据强造。设计软件直连仍不在本批范围；未提交、推送或部署，Task 8 保持 `NOT_READY`。

2026-08-24 D-201 已发布正式站：客服“产品目录”和管理端“产品总览”只读使用医生端当前已发布下单目录，新增／修改统一由管理端“下单内容设置”维护；医生端材料／色号选填、色号搜索与自定义、口扫批量分类上传、精确提示定位，以及色号候选层和移除确认层遮挡问题均已收口。PR #46／#47 发布正式版本 `01919fb61c6582d2e8d3e124ceddcce3b613749f`，自动部署 #32698278453 成功；完整后端 357 项、release gates、镜像校验、备份、部署、健康检查和公网探针全部通过。正式站只读复验确认后端健康为 `ok`，管理端当前发布目录显示 95 个产品，并明确新增或修改进入“下单内容设置”；未写正式业务数据，Task 8 保持 `NOT_READY`。

2026-08-21 医生下单步骤缺项反馈已补齐：点击“下一步”或“提交订单”时，页面持续列出步骤、产品和具体缺失字段／资料，并提供“去填写”定位；最终按钮不再因缺项静默禁用，但前后端提交门禁保持不变。本地构建、专项检查和隔离浏览器交互验收通过，未部署，Task 8 保持 `NOT_READY`。

2026-08-21 医生多产品资料上传已按共享默认／产品专属覆盖补强：病例共享区和每个产品专属区使用相同六类槽位；每个产品的上颌、下颌、咬合扫描可由共享或专属资料满足，后三类选传。产品专属同类文件只覆盖当前产品，未上传时明确显示继承的共享文件。新草稿使用 `FIXED_LAYERED_V2`，历史 `FIXED_SHARED_V1` 继续兼容；本地前端构建、专项检查、全新隔离数据库目标测试及隔离浏览器的共享实传／双产品继承／单产品覆盖／最终摘要均通过，最终提交未点击，未部署，Task 8 保持 `NOT_READY`。

2026-08-18 医生端英文界面已按 D-197 完整收口并发布正式站：共享登录页按用户确认保持中文，进入医生端后才应用中文／`EN` 切换。最终正式版本 `f64b951155926df48b5c20afa58ae5e4a2a77212`，自动部署 #32056401585 结果 `success`；后端 351 项测试、release gates、镜像校验、备份、部署、健康检查和公网探针全部通过。线上只读浏览器除覆盖 7 个主页、下单子流程和动态产品字段外，还按反馈截图复验了公开进度 8 个节点、草稿说明、公开进度空提示、订单时间线、通知标题／系统摘要和物流事件，均为英文。患者姓名、诊所名、上传文件名、承运商名、医生输入及消息正文等原始业务数据保留原文；未写入正式业务数据，Task 8 保持 `NOT_READY`。

2026-08-16 外部黑盒回归报告的 8 个 OPEN 项已按当前源码重新分类并发布：BUG-013 已由既有并发保存保护覆盖，其余 7 项修复经 PR #21/#22 进入正式版本 `54eea0e4399782485d7953231e4768ed684cd097`，自动部署运行 #31939878141 成功。全新隔离库后端 350 tests、前端/OpenAPI/发布门禁和本地 smoke 通过；正式站只读回归确认四端刷新保持登录、客服 390px 无溢出、生产帮助和扫码登记入口可用。没有生产任务或附件草稿时不写正式数据强造用例；Task 8 保持 `NOT_READY`。
...
2026-08-01 已按重新提供的《生产流程》恢复“工序工时设置”草稿入口：现有九条工序链不变，V74 恢复管理员菜单，并确保存在全节点空值草稿供逐项填写、批量保存或 CSV 导入。未填写项显示“待设置”；正式运行时开关仍默认关闭，页面和后端均禁止把草稿发布到新实例、交期/超时、产能或绩效。17/19 分钟验收版本保持 INACTIVE，Task 8 保持 `NOT_READY`。
