...
2026-08-17 已按 D-197 完成医生端中英文界面本地收口：英文模式覆盖工作台、订单、助手、患者、账单物流、消息、账户诊所及相关抽屉／对话框，并贯通六步下单向导、动态产品字段和隐形正畸七步处方。患者名称、诊所名称、医生输入和历史消息保留原文；本次不改后端、数据、权限或脱敏边界。`npm run check:doctor-portal-i18n`、原医生端／产品优先下单检查、前端生产构建和差异检查已通过；当前尚未合并或部署，Task 8 保持 `NOT_READY`。
