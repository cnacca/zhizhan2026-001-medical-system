...
2026-08-17 医生端中英文界面已按 D-197 发布正式站：PR #27/#28 已合并，正式版本 `78c0955b35f519f77bf28b107e8ae5a1b138cfd6`，自动部署运行 #32033871635 结果 `success`。英文模式覆盖 7 个主页、抽屉／对话框、六步下单向导、动态产品字段和隐形正畸七步处方；患者名称、诊所名称、医生输入和历史消息保留原文。正式站首页返回 200，线上实际医生端 JS/CSS 包已核对英文文案、语言记忆键和可切换标签；当前浏览器读取通道超时，未把登录后真实点击记为已完成。本轮不改后端、数据、权限或脱敏边界，Task 8 保持 `NOT_READY`。
