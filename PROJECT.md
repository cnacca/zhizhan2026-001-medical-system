...
- 生产端数据审核、工序任务池、入检、出检、返工、工时、绩效、终检、人员档案 / 工作量看板、专项质量管理。
...
- 不继续扩展设备管理、物料异常管理、安环管理、成本管理、奖惩模块的完整录入 / 审批 / CRUD / 真实趋势；最新版 PRD 将这些列为二期，当前已做只读汇总只能作为超一期展示或二期储备。
...
1. 患者管理基础版第一增量：患者档案、订单绑定、本人数据隔离、列表检索和医生端入口。
2. 人工支付流水 / 收支记录第一增量：补齐 PRD 一期“基础支付流水和账单查看”，不接真实支付网关。
3. 客户 / 诊所档案与偏好第一增量：支撑客服客户管理和医生下单偏好提示。
4. 人员档案 / 工作量看板第一增量：复用现有用户、部门、岗位、工时和绩效事实。
5. 质量记录 CRUD / 外返登记第一增量：补 `quality_record`、内返 / 外返状态处理和质量统计验收。
