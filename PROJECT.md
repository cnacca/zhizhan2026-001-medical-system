...
- 客服端订单初审、AI 翻译、资料缺失检查、客户档案与偏好、产品参数 / 价格体系维护、订单消息审核、设计稿审核、账单 / 人工支付流水、物流录入 / 异常跟进、客服查询助手；外协管理按 2026-07-06 基准纳入一期基础登记和状态，完整成本核算另行评估。
...
1. P0 安全：封堵医生访问内部 workflow / 生产接口，并统一无 token 订单读取边界。
2. P0 主链：补工序链自动匹配、医生确认设计稿生产门禁和出检通过后继激活门禁。
3. P0 管理：补管理员创建 / 编辑技工账号、部门岗位和可登录闭环。
4. 按原 PRD 38行的 `PARTIAL` 项与 2026-07-06 A/B/C 表外范围继续验收和收口。
5. 并行收集 2 项产品确认、AI-5 模板、标准工时业务数据，并准备 AI / 文件 / 通知 / 部署真实环境与最终总体验收证据。
