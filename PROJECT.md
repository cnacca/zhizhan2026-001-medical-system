...
- 2026-08-25 D-214：“推簧”经专业资料核验为真实正畸术语，不按“弹簧”错别字归并；用户决定当前暂缓，记录为 `USER_DEFERRED`，不进入当前实现或验收。TASK-039 E 只补螺旋扩弓器单向／双向，D-211 错误别名实现须在任何提交前清除，Task 8 保持 `NOT_READY`。
- 2026-08-25 D-213：已把《动态下单表最终版》12 页内容固化为来源快照。原表没有“推簧”，只分别定义产品“弹簧矫正器”、产品“螺旋扩弓器（单向）/（双向）”和附件“加弹簧”；D-211 的别名推断证据不足并重新打开。当前目录还丢失螺旋扩弓器方向信息，GOAL-037／TASK-039 恢复 `in_progress`，不得提交或发布错误别名实现，Task 8 保持 `NOT_READY`。
- 2026-08-25 D-212：当前指纹的《动态下单表最终版.docx》明确要求设计服务选择数据格式 `STL／OBJ／EXO／3SHAPE`、设计标准“通用／个性化”和设计时间“12 小时／24 小时／3 天”。三项已恢复为医生端必选并由 V94 后端目录规则校验；D-210 备注选填与 D-208 数字交付边界继续有效。尚未提交发布，Task 8 保持 `NOT_READY`。
- 2026-08-25 D-211（历史错误判断，已被 D-213 重新打开）：曾把“推簧”映射为 `ORTHO_SPRING_APPLIANCE / 弹簧矫正器` 的检索别名；该实现未提交、未推送、未发布，不得作为当前依据。
- 2026-08-25 D-207：用户将完整 CRM／客户关怀提醒和 STL 直接打开 CAD／设计软件转入二期；一期按 GOAL-037／TASK-039 先做医生下单 6 → 4 步精简，再核对“推簧”目录映射。4 步只合并页面分组，不删除字段、动态表单、正畸处方、文件规则或提交门禁；“推簧”在来源核对前不猜测落目录。本轮仅完成实施文档，尚未修改业务代码，Task 8 保持 `NOT_READY`。
- 2026-08-25 D-206：用户暂不提供 AI-5 正式生产备注模板、各工序标准工时、正式 AI Key／预算上限／告警接收人；统一标记为 `USER_DEFERRED`，在用户主动恢复前不再询问或催办。相关真实验收不伪装完成，正式标准工时和真实 AI 配置继续关闭，Task 8 保持 `NOT_READY`。
- 2026-08-25 D-205 校准客户输入：动态表单与文件要求此前均已提供，CP-002／CP-005 改为 `CONFIRMED_BASELINE`，不再重复索取。文件基线为单文件 500MB、单订单最多 50 个及类型／产品资料规则；后端／测试默认、前端限制、开发／生产 env 模板、Compose 和完整发布后容器复核已统一到 50，本地修复待提交／发布。AI-5 模板、标准工时和真实环境验收仍未完成，Task 8 保持 `NOT_READY`。
...
- 2026-08-01 按 D-182 将隐形正畸从无来源依据的“A 型”占位口径纠正为 `CLEAR_ALIGNER_BRACELESS / 无托槽隐形矫治器`，医生多产品向导已接通牙颌、常规／联合矫治、资料上传和既有七步处方；A 型继续停用，历史订单不改写，价格仍待报价。
...
- 2026-08-25 验收算术校准：GOAL-023 / TASK-024 的本地收口事实不变；原 PRD 38 行实际状态为 29 PASS、1 PARTIAL、0 MISSING、8 EXTERNAL_ACCEPTANCE，修正旧摘要 30／1／0／7。标准工时业务数据与真实环境验收仍未完成，Task 8 保持 `NOT_READY`。
...
- 2026-08-25 D-215：螺旋扩弓器已在本地以必选产品规格补“单向／双向”，医生端、服务端校验和订单快照均保留方向；V92／V93 及错误推簧别名实现已移除，V95 清理开发库残留。GOAL-037／TASK-039 本地完成，未提交、推送或部署，Task 8 保持 `NOT_READY`。
