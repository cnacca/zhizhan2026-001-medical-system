...
- 2026-08-16 D-193 已按确认边界收口：隔离 demo seed 为四个验收账号提供各端验收角色；真实账号由细分角色决定权限与 DataScope；设计技术内审归生产侧获授权组长。物流、QC、专项动作端口归属、RBAC 角色家族和大范围导航映射等未确认改动已撤回，等待产品确认后再处理。全新隔离库定向后端 27 tests、前端生产构建、OpenAPI、验收 RBAC、前端缺陷审计与差异检查通过；默认测试库既有 V73 checksum 漂移未 repair，未改正式数据、未提交或部署；Task 8 保持 `NOT_READY`。

- 2026-08-15 D-192 保留不依赖新增业务口径的技术修复：Refresh Token family／退出并发、WebSocket ACTIVE 复核、医生写权限、Multipart/STL 并发边界、前端跨账号状态／Mock，以及 Compose/OpenAPI/生产发布门禁。物流共享范围、物流状态拆分和工作流 QC 投影不再列为已完成事项；本批不执行生产数据变更、Flyway repair、提交或部署。
