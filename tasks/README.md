...
- 2026-08-16：外部黑盒回归报告的 8 个 OPEN 项已按当前源码重新分类并完成本地收口。BUG-013 已由既有同步互斥锁覆盖；BUG-011、020、025、026、027、028、029 已补附件刷新回显/软删除、扩展名白名单、HttpOnly cookie 会话恢复、客服窄屏、生产帮助和扫码入检竞态保护。全新隔离库后端 350 tests、前端构建、OpenAPI、既有专项回归、本批静态检查及 390px 浏览器 smoke 通过，隔离库和临时授权已清理，新专项检查已纳入正式发布门禁。报告没有被测 Git SHA，故结论仅适用于当前工作树；尚未提交、合并或部署，外部环境仍需按原用例复测。未改可见性业务口径，不物理删除历史文件，Task 8 保持 `NOT_READY`。
