...
- 2026-09-02：按 D-217 完成订单后续需求本地实现。范围包括上传白名单全预览、四端工作台业务跳转、医生批量上传与自动分类、阶段化补充资料、仅系统预计到货日期、客服初审生产单号、发货前盒号唯一、生产组长本部门派工／改派、医生内部节点隔离、设计确认与沟通拆分及发货 7×24 小时自动确认。全量后端 368 项测试、前端生产构建、OpenAPI 和专项门禁通过；真实 Edge 已验证四端工作台关键跳转、客服 STL 预览／交期维护、生产组长权限入口与工序级派工界面，未执行真实派工写入。15 类文件全组合、补充资料各阶段和 7 天时间推进仍以自动化证据为主。未提交、推送、部署或修改正式业务数据，Task 8 保持 `NOT_READY`。

- 2026-08-25：GOAL-037／TASK-039 已经 PR #55／#56 发布为正式版本 `78d68ec17f7e0c1d7f625f0594cc21c6ab07773c`。自动部署 #32843001050 按 `full` 通道用时 5m50s，后端 362 项、release gates、双镜像、发布包、MySQL 备份、双容器替换、健康检查和公网探针全部通过；公网首页 200、无效登录 401、别名域名 308 到规范域名。医生四步下单、设计服务来源字段与数字交付、备注选填、字段归属去重、螺旋扩弓单向／双向和 V89-V95 已上线；未写正式订单，“推簧”继续 `USER_DEFERRED`，Task 8 保持 `NOT_READY`。

- 2026-08-25：TASK-039 E 批次按 D-213 重新打开。《动态下单表最终版》12 页来源内容已固化；原表有“弹簧矫正器”“螺旋扩弓器（单向）/（双向）”和附件“加弹簧”，没有“推簧”。测试报告栏目不能证明同义关系，D-211 的“推簧”产品别名实现不得提交或发布；当前目录还丢失螺旋扩弓器单向／双向信息。GOAL-037／TASK-039 恢复 `in_progress`，尚未提交、推送、发布或写正式数据，Task 8 保持 `NOT_READY`。

- 2026-08-25：TASK-039 D3 已按 D-212 完成。当前指纹的《动态下单表最终版.docx》XML 与渲染第 4 页均确认设计服务三项制作要求：数据格式 `STL／OBJ／EXO／3SHAPE`、设计标准“通用／个性化”、设计时间“12 小时／24 小时／3 天”。医生端三项恢复为必选，V94 为全部既有设计服务目录版本补规则并由后端校验必填与允许值；备注继续选填，牙位、通用口扫、数字设计、医生确认和付款下载门禁不变。`OrderCaseGroupTests` 11 项、专项检查、i18n、前端生产构建和差异检查通过；尚未提交、推送、发布或写正式数据，Task 8 保持 `NOT_READY`。

- 2026-08-25【历史错误口径，已由 D-213 撤销】：GOAL-037／TASK-039 E 批次曾按 D-211 把“推簧”映射为 `ORTHO_SPRING_APPLIANCE / 弹簧矫正器` 的检索别名。该结论证据不足，V92／V93、活动目录别名返回和医生搜索均不得提交或发布；当前状态以本页顶部 D-213 记录为准。

- 2026-08-25：TASK-039 第 1 项追加调整已按 D-210 完成。设计服务备注改为选填且没有新增选项，V91 同步取消全部现有目录版本的服务端必填；牙位、通用口扫、设计确认和付款下载门禁不变。复核页上下区块已统一右侧内容基线与宽度。`OrderCaseGroupTests` 11 项、四步设计服务专项、字段归属检查、前端生产构建和差异检查通过；浏览器只读恢复现有草稿，未保存或提交。未提交、推送或热更新，Task 8 保持 `NOT_READY`。

- 2026-08-25：TASK-039 第 1 项追加缺陷已按 D-209 收口。全部产品类别完成字段归属审计：种植参数不再在“制作要求”和“材料与工艺”重复；固定修复两个边缘维度已明确区分；动态补充字段会排除页面固定字段并按 key 去重；设计服务不显示空材料区。新增 `check:doctor-order-field-ownership`，医生下单专项、目录与前端回归和生产构建通过。本地浏览器只读确认向导正常加载且控制台无错误，未创建患者或订单草稿。未提交、推送或热更新；TASK-039 继续 `in_progress`，剩余 E 批次“推簧”目录映射，Task 8 保持 `NOT_READY`。

- 2026-08-25：GOAL-037／TASK-039 第 1 项完成本地实现与回归。医生端已由 6 步合并为 4 步，并同步导航、缺项定位、最终确认和服务端错误映射。设计服务按 D-208 使用通用扫描资料、牙位和设计要求备注，不再显示实体出货／运输、材料工艺或试戴流程；医生确认后数字工作流完成，账单付款后才可下载原文件。前端生产构建、专项与既有检查通过，后端三个目标测试类 43 项通过，本地医生端浏览器确认四步和数字交付文案且控制台无错误，未创建或提交订单。未提交、推送或热更新；TASK-039 继续 `in_progress`，剩余 E 批次“推簧”目录映射，Task 8 保持 `NOT_READY`。

- 2026-08-25：按 D-207 建立 GOAL-037 / TASK-039。用户将完整 CRM／客户关怀提醒和 STL 直接打开 CAD／设计软件转入二期；一期先做医生下单 6 → 4 步精简，再核对“推簧”目录映射。实施文档固定目标 4 步为“基础信息与产品、制作配置、资料上传、复核与提交”，当前 6 步按 `1 → 1`、`2 + 3 → 2`、`4 → 3`、`5 + 6 → 4` 合并，现有字段、动态表单、正畸处方、文件规则、缺项定位和前后端提交门禁不删除、不放宽。“推簧”核对前不新增／改名产品。本轮只完成范围与实施文档，业务代码、功能回归、提交、推送和部署均未执行；Task 8 保持 `NOT_READY`。

- 2026-08-25：TASK-038 一期验收基线校准完成。以正式版本 `7013b1434759df6ca77a65893dbdcf0129e9af7b`、自动发布 #32742027332、线上只读复验和当前代码为准，修正 `docs/INDEX.md`、`docs/DELIVERY-GAP.md`、`acceptance.json` 和 9D.81 的陈旧冲突。正式部署、HTTPS、医生端英文、MySQL 发布前备份和 D-202 站内文件查看不再写成未完成；恢复／回滚／监控、MinIO 备份、真实 AI／文件／WebSocket、用户暂缓输入和最终交付仍保持 `PARTIAL`。用户已将 AI-5 正式模板、标准工时、正式 AI Key／预算上限／告警接收人标记为 `USER_DEFERRED`，主动恢复前不再询问或催办。逐行复核发现旧 30／1／0／7 摘要算术错误，原 38 项准确为 29 PASS、1 PARTIAL、0 MISSING、8 EXTERNAL_ACCEPTANCE。新增 `check:acceptance-baseline` 并校准旧验收／9D.81 检查；未写正式数据、未触发热更新，Task 8 保持 `NOT_READY`。

- 2026-08-24：D-203 正式发布提速已经 PR #52／#53 发布为正式版本 `7013b1434759df6ca77a65893dbdcf0129e9af7b`。`Deploy production` 新增 fail-closed diff 分类；纯 `frontend/**` 加受控文档走前端快速通道，只构建、上传、校验和替换前端镜像，其他改动与手工发布保持完整后端测试、双镜像、数据库备份和双容器更新。快速通道仍保留 PR 门禁、专项回归、SHA-256、known_hosts、部署锁、前端回滚标签、后端／首页健康检查和公网探针。首次发布因包含工作流／脚本自动走 `full`；自动部署 #32742027332 用时 10m21s，后端 358 项、release gates、双镜像、MySQL 备份、部署和公网探针全通过，部署后首页／健康接口均为 200。首个真实纯前端耗时尚待下一次符合条件的发布实测；未修改正式业务数据，Task 8 保持 `NOT_READY`。

- 2026-08-24：D-202 阻断修复已经 PR #49／#50 发布为正式版本 `e19bce4d70e672920d6d3c5430f9743946e96d4a`。客服 STL 查看器已移到门户全局层；管理端预览按 STL、图片／PDF、其他格式站内分流，不再用空白页导航或自动下载；MinIO 预览／下载签名分别使用 `inline`／`attachment`。生产审核员的额外附件读取仍严格限定为 `PENDING_PRODUCTION_REVIEW`，上传／完成写操作与审核后未派工文件仍返回 403。自动部署 #32731578654 成功通过完整后端 358 项、release gates、不可变镜像构建／校验、发布包、上传、备份／部署／健康检查和公网探针；部署后首页返回 200、后端健康为 `ok`，线上前端产物包含“浏览器 3D 查看”新链路。隔离浏览器 smoke 为 3/3，已断言 canvas 成功生成、只请求预览、不请求下载且不新增页面；真实普通生产账号因无分配工单，在线点击证据保持 `PARTIAL`，未写正式数据强造。Task 8 保持 `NOT_READY`。

- 2026-08-24：D-201 已经 PR #46／#47 发布为正式版本 `01919fb61c6582d2e8d3e124ceddcce3b613749f`。客服产品页与管理端产品总览同源读取已发布目录，维护入口归管理端“下单内容设置”；医生材料／色号选填、色号搜索与自定义、`CLASSIFIED_SCAN_BUNDLE_V3` 批量分类上传、提示定位及浮层遮挡修复均已上线。自动部署 #32698278453 成功通过完整后端 357 项、release gates、镜像校验、发布包、备份／部署／健康检查和公网探针；正式站只读复验确认后端健康为 `ok`，管理端显示当前发布目录 95 个产品及正确维护入口。未写正式业务数据，Task 8 保持 `NOT_READY`。

- 2026-08-21：GOAL-036 / TASK-037 追加完成医生下单缺项反馈。点击“下一步”后持续显示步骤、产品和具体缺项，并提供“去填写”定位；资料缺项明确到产品和扫描类型。最终提交按钮允许点击后解释全部缺项，但不放宽前端或后端提交门禁。前端生产构建、专项检查、i18n、产品优先检查、acceptance 和差异检查通过；隔离浏览器验证第一步、产品字段、定位回填和最终确认缺项反馈，控制台无错误，未提交订单。未提交/推送/部署，Task 8 保持 `NOT_READY`。

- 2026-08-21：按用户追加确认扩展 GOAL-036 / TASK-037，病例共享区与每个产品专属区统一为六类资料；每个产品的上颌、下颌、咬合扫描可由共享或专属资料满足，后三类选传。同类专属文件仅当前产品优先使用，未上传专属时显示继承的共享文件。`FIXED_LAYERED_V2` 后端校验与 `FIXED_SHARED_V1` 历史兼容已完成；前端生产构建、专项检查和全新隔离数据库 `OrderCaseGroupTests` 6/6 通过。本地隔离浏览器已验证 3 个共享 STL 对两个产品的继承、单产品专属 STL 覆盖及最终页附件摘要，控制台无错误；未点击最终提交。未提交/推送/部署，Task 8 保持 `NOT_READY`。

- 2026-08-18：GOAL-036 / TASK-037 医生下单最终确认与共享资料上传已完成本地开发。三个扫描槽位必传、其他选传，多产品共享一组病例文件；最终页完整展示八类确认信息，价格未配置时仅显示占位；询问客服不建状态机、不形成提交门禁。自动化检查、前端构建、后端编译、隔离库后端目标测试 5/5 通过；真实 Chrome 上传被本地 MinIO bucket HTTP 503 阻断，浏览器全路径为 `PARTIAL`。未提交/推送/部署，Task 8 保持 `NOT_READY`。

- 2026-08-18：医生端英文界面已按 D-197 完整收口。共享登录页保持中文，动态系统文案补漏经 PR #38/#39 发布为 `f64b951155926df48b5c20afa58ae5e4a2a77212`，自动部署 #32056401585 成功通过后端 351 项测试、release gates、镜像校验、备份、部署、健康检查及公网探针。正式站按反馈截图只读复验确认公开进度 8 个节点、草稿说明、公开进度空提示、订单时间线、通知标题／系统摘要和物流事件均为英文；上传文件名、患者／诊所／承运商名称和消息正文等原始业务数据保留原文。未写正式业务数据，Task 8 保持 `NOT_READY`。

- 2026-08-16：外部黑盒回归报告的 8 个 OPEN 项已完成发布。BUG-013 复核为当前源码既有同步互斥锁；BUG-011、020、025、026、027、028、029 的附件回显/软删除、扩展名白名单、HttpOnly cookie 会话恢复、客服窄屏、生产帮助和扫码入检竞态保护经 PR #21/#22 发布为 `54eea0e4399782485d7953231e4768ed684cd097`，运行 #31939878141 成功。正式站只读回归确认四端刷新保持登录、客服 390px 无溢出、生产帮助和扫码登记入口可用且网站控制台无 error/warning；无生产任务和附件草稿时不写正式数据强造用例，写路径继续以隔离库 350 tests 和本地回归为证据。未改可见性业务口径，不物理删除历史文件，Task 8 保持 `NOT_READY`。

- 2026-08-16 D-192／D-193 已通过 PR #19、PR #20 发布正式站，最终版本 `e465d8d8d13b30c2e6d265fea3e3f82e55b2e9ec`。首轮发布在服务器变更前被跨测试顺序污染门禁阻断；夹具隔离修复后随机顺序后端 348 tests 全绿，运行 #31937188201 完成镜像校验、备份／部署、容器健康、公网 CORS 与规范入口检查。正式浏览器只读验证四端测试账号均可登录对应门户，未写业务数据；Task 8 继续保持 `NOT_READY`。

- 2026-08-16 D-193 已按确认边界收口并进入 `dev`：隔离 demo seed 为四个验收账号提供各端验收角色；真实账号由细分角色决定权限与 DataScope；设计技术内审归生产侧获授权组长。物流、QC、专项动作端口归属、RBAC 角色家族和大范围导航映射等未确认改动均未带回。全新隔离库后端 348 tests、前端生产构建、OpenAPI、验收 RBAC、认证／退出、部署／Compose、acceptance 与差异检查通过；未改正式数据、未重播现有 demo、未合并 `main` 或部署；Task 8 保持 `NOT_READY`。

- 2026-08-15 D-192 保留不依赖新增业务口径的技术修复：Refresh Token family／退出并发、WebSocket ACTIVE 复核、医生写权限、Multipart/STL 并发边界、前端跨账号状态／Mock，以及 Compose/OpenAPI/生产发布门禁。物流共享范围、物流状态拆分和工作流 QC 投影不再列为已完成事项；实现已进入 `dev`，不执行生产数据变更、Flyway repair、`main` 合并或部署。
...
- 2026-08-15：正式站登录 CORS 缺陷已完成生产止血与永久防复发实现。运行日志确认失败浏览器来自 `www` 域名及直接 `:8088` 入口，生产后端当时只允许裸域名，且容器在故障时段没有重启。生产环境已补入受控 `www` Origin 并重建后端；代码侧新增 `www`／直接端口到裸域名的 308 规范化，以及发布后两个登录 Origin 和两个入口的公网探针。部署环境、自动发布和差异检查作为本批门禁；本项不改变 Task 8 `NOT_READY`。
...
2026-08-25 校准：GOAL-023 / TASK-024 的默认 Bearer、质量趋势、设计稿多版本、动态表单和隔离真实 STL 12 步主链事实不变；原 PRD 38 行实际状态为 29 PASS、1 PARTIAL、0 MISSING、8 EXTERNAL_ACCEPTANCE，修正旧摘要 30／1／0／7。CP-004 标准工时和真实环境验收仍待外部输入，Task 8 保持 `NOT_READY`。
...
- Task 8 仍保持 NOT_READY；下一步推荐 9D.95.3 安环巡检 / 隐患整改第一增量。
- 2026-08-25：用户按 D-214 将“推簧”移出当前实施与验收范围。外部专业资料确认“推簧”是真实正畸术语，不按“弹簧”错别字归并；其目录身份保持 `USER_DEFERRED`，用户恢复前不再追问。D-211 的未提交错误别名仍须在任何提交前清除；TASK-039 E 当前只补螺旋扩弓器单向／双向及订单快照，Task 8 保持 `NOT_READY`。
- 2026-08-25：GOAL-037／TASK-039 已按 D-215 完成本地实现。螺旋扩弓器保留单一产品，通过必选“单向／双向”规格表达方向，医生端、服务端提交门禁和订单快照已接通；错误推簧别名实现及 V92／V93 已移除，V95 清理可能残留的数据。“推簧”继续 `USER_DEFERRED`。后端 21 项、目录与医生下单专项、i18n、验收基线、前端构建和浏览器只读检查通过；未提交、推送或部署，Task 8 保持 `NOT_READY`。
