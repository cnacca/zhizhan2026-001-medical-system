...
- 2026-08-17：医生端英文界面已按 D-197 完成本地收口。7 个主页、详情与编辑对话框、六步下单向导、动态产品字段及隐形正畸七步处方共用同一语言状态，浏览器记住中文／`EN` 选择。新增 `check:doctor-portal-i18n`，原医生端与产品优先下单检查、前端生产构建和差异检查通过。患者／诊所名称、医生输入和历史消息仍按原文显示；本轮不改后端、数据、权限或脱敏，尚未合并或部署，Task 8 保持 `NOT_READY`。
