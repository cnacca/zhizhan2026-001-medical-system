...
- 2026-08-14：客服新增客户 403 局部权限修复已部署到正式站，不另建 RepoFrame task。线上已幂等补授 `CS -> clinic:create`，权限表与旧前端镜像均有回滚备份；正式前端切换到 `fix/cs-clinic-create-permission@24cfd5dc` 的验证产物。公网新资源、健康接口、CORS、一期容器和权限记录复核通过；使用短时 CS 令牌及空请求体调用 `POST /clinics` 返回 400 而非 403，证明已通过权限门禁且未写入客户数据。仍需客服重新登录后完成真实建档复测，Task 8 保持 `NOT_READY`。

- 2026-08-14 交接冻结：按用户要求，本轮不继续实现取消订单完整闭环，已将当前事实和后续任务完整写入 `docs/development/doctor-order-delete-cancel-handoff-20260814.md`。后续窗口必须先处理多产品病例草稿入口一致性，再确认取消阶段／处理角色／审批后报价、设计、生产、账单、退款、物流规则；随后另开任务实现客服／管理员待审列表、同意／拒绝、正式“已取消”状态、医生结果反馈、通知、撤回／重提（如确认）、审计及并发／权限回归。当前功能改动仍未提交、未合并、未发布到正式站点；正式域名的 HTTPS 基础已完成，但业务复测、防火墙收口和恢复演练仍未关闭。Task 8 保持 `NOT_READY`。

- 2026-08-14：医生订单草稿删除／审核中取消申请已作为局部开发完成，不另建 RepoFrame task。草稿支持操作列单条软删除和草稿筛选下最多 50 条原子批量软删除；客服或生产待审核订单支持带原因的幂等取消申请，不删除订单、不自动改变状态、不开放批量取消。V84、后端权限／审计、Doctor Portal V2 和 OpenAPI 已接通；全新隔离库 V1→V84、29 项组合回归、补充后的 24 项订单专项测试、前端生产构建和 OpenAPI 校验通过。尚未发布到正式站点，真实医生账号复测前不得写成线上已完成，Task 8 保持 `NOT_READY`。
