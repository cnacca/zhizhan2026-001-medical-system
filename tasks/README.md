...
- 2026-08-24：按 D-203 完成正式发布提速本地实现。`Deploy production` 新增 fail-closed diff 分类；纯 `frontend/**` 加受控文档走前端快速通道，只构建、上传、校验和替换前端镜像，其他改动与手工发布保持完整后端测试、双镜像、数据库备份和双容器更新。快速通道仍保留 PR 门禁、专项回归、SHA-256、known_hosts、部署锁、前端回滚标签、后端／首页健康检查和公网探针。新增 `check:production-fast-deploy` 覆盖 9 类分流场景；已在修复分支完成验证，尚未合并或部署，Task 8 保持 `NOT_READY`。
