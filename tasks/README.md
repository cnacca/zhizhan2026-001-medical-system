...
- 2026-08-24：按 D-201 扩展 GOAL-036 / TASK-037，并已获用户授权进入正式发布流程。客服产品页与管理端产品总览均改为读取医生端已发布目录的只读视图，维护入口归管理端“下单内容设置”；当前发布版本显示 95 个启用产品，旧产品表单条种子数据不再作为目录。医生端主材料／制作项目／牙色系统／色号改为选填，色号支持搜索、筛选和自定义输入，并修复色号候选层、移除确认层被订单向导遮挡的问题；口扫改为 `CLASSIFIED_SCAN_BUNDLE_V3` 批量上传，完整资料包或上颌＋下颌＋咬合三类齐全才可提交，无法区分的散文件和 PDF 不算完整；所有可修正提示带对应步骤／产品／区域定位。全新专用测试库后端 357 项、前端生产构建、后端 package、正式发布门禁和专项检查全部通过，隔离浏览器已验证本批三项反馈且控制台无错误。尚未提交／推送／部署，Task 8 保持 `NOT_READY`。
