...
- 按 D-182 启用来源可追溯的“无托槽隐形矫治器”，支持未来扩展更多类型；联合矫治以同组关联子订单表达。
- 复用现有设计文件/版本能力，不引入高级 Web 3D。

Acceptance：

- [x] “无托槽隐形矫治器”由发布配置启用，A 型占位项保持停用且医生端不展示。
- [x] 医生端可选择全颌／上颌／下颌、常规／联合矫治，并完成七步处方后提交病例订单。
...
- 2026-08-01 / F（D-182）：核对正畸平台医生端文档后，V76 复制当前发布目录并新增 `CLEAR_ALIGNER_BRACELESS / 无托槽隐形矫治器`；医生端开放隐形正畸分类、牙颌／矫治方式、资料槽位和七步处方，A 型继续停用，价格保持待报价。隔离环境真实浏览器已验证医生选择该产品、全颌和常规矫治后可连续进入七步处方，三个选择值均正确回填。
