...
- [x] 口扫资料改为同一区域一次多选 ZIP／STL／PLY／OBJ，并按文件名自动分类、允许手工改为上颌／下颌／咬合／完整资料包。
- [x] 新订单按“完整资料包”或“上颌＋下颌＋咬合齐全”校验；无法分类的散文件和 PDF 不计入完整性，历史 V1／V2 草稿保持兼容。
- [x] 主材料、制作项目、牙色系统和色号改为选填；色号支持输入筛选现有选项和保存自定义值。
- [x] 所有可修正提示携带定位目标；点击后切换步骤、选择产品并聚焦修改区域，系统错误不显示伪定位。
- [x] 客服产品页读取已发布目录并只读展示，明确由管理端“下单内容设置”维护。

Acceptance：批量上传不牺牲生产角色可区分性；一个 ZIP 或分类齐全均可满足，每个未识别散文件必须由医生校正；医生可不填本次确认的材料／色号项；提示能到达实际可修改区域。

Verification：专项检查、前端生产构建、后端编译和新增集成测试；全新专用测试库完整后端 357 项通过。
...
- 已发布产品目录可同时作为医生下单和客服只读产品目录的数据源；管理员已有草稿／发布维护入口。
...
- “把多份扫描文件放在一个上传框就自然能供生产区分”不成立；必须保存每个文件的角色分类，或上传本身完整的资料包。
