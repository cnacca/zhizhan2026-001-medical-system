...
- 2026-08-17 医生端中英文界面已按 D-197 发布正式站：PR #27 合并 `dev`、PR #28 合并 `main`，正式版本 `78c0955b35f519f77bf28b107e8ae5a1b138cfd6`。自动部署运行 #32033871635 用时 5m44s，完成后端全量测试、release gates、不可变镜像构建／校验、发布包、生产备份／部署／健康检查和公网 Origin／规范入口探针，结果 `success`。英文模式覆盖 7 个主页、抽屉／对话框、六步下单向导、动态产品字段和隐形正畸七步处方，语言选择持久化；患者／诊所名称、医生输入和历史消息保留原文。正式站首页返回 200，线上实际医生端 JS/CSS 包已核对英文关键文案、语言记忆键和可切换 CSS 标签。当前浏览器读取通道超时，未把登录后真实点击记为已完成；本轮未写正式业务数据，未改后端、权限或脱敏边界，Task 8 保持 `NOT_READY`。
