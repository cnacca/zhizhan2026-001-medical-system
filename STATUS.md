...
- Active task: `tasks/README.md` 的「任务 8：专项验收矩阵与上线准备」，状态为 `in-progress/next-webhook-replay-protection`
...
下一轮唯一推荐目标：AI 外部告警接收端 webhook 验签/防重放第一增量。基于 9D.47 发送侧 HMAC 签名和 9D.48.2 失败可见性，补接收端签名校验、timestamp/nonce/replay window 的最小静态/后端验收边界，不接真实外部服务、不提交真实 secret。Task 8 总体仍保持 `NOT_READY`，直到生产级防重放、真实弱网/跨设备续传、终检专用角色/附件、完整返工闭环、绩效完整公式/周期/申诉/标准工时配置、AI 治理闭环和部署交付材料补齐。
