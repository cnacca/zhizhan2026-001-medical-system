...
- 2026-08-24 已按 D-203 完成正式发布提速的本地实现：保留单一 `Deploy production` 入口，自动比较 main merge commit 与第一父提交；只有 `frontend/**` 加受控发布文档时走 `frontend` 快速通道，其他改动和手工发布全部 fail closed 为 `full`。快速通道跳过后端测试基础设施、后端全量测试／打包／镜像、MySQL 备份和后端重启，只上传并替换经校验的前端镜像；PR 校验、前端回归门禁、SHA-256、known_hosts、部署锁、前端回滚标签、双健康检查和公网探针继续保留。新增 `check:production-fast-deploy` 覆盖前端、文档、后端、迁移、根锁文件、工作流、部署脚本和空 diff 分类；已在修复分支完成验证，尚未合并或部署，Task 8 保持 `NOT_READY`。
