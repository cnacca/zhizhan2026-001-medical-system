...
- 2026-08-24 已按 D-202 完成《2026.8.22 软件测试报告》一期阻断项及本轮预览反馈的本地修复与回归校准：客服信息审核页补齐可解释的完整性依据、明确退回原因和“退回补资料”闭环；生产审核员仅在订单处于 `PENDING_PRODUCTION_REVIEW` 时获得附件列表／预览／下载权限，文件写操作和审核后的未派工订单继续禁止；客服附件查看器改为门户全局挂载，信息审核页点击“3D 查看”可正常渲染；管理端预览不再打开空白页或触发下载，STL 进入站内 3D 查看，图片／PDF 站内预览，其他格式明确提示后由用户选择下载；MinIO 预览／下载签名分别固定为 `inline`／`attachment`，等待仍有 15 秒超时。旧 `check:feedback-20260713` 已改为要求站内查看器并禁止 `about:blank`／`popup.location.replace`；`smoke:feedback-20260822-blockers` 新增普通 `WORKER`＋`SELF` 生产看板回归，断言 STL 实际解析并生成 canvas、预览接口只调用一次、下载接口不调用且不新增标签页，现为 3/3。`FileAccessTests` 18/18、前端生产构建、专项静态检查及生产看板既有检查通过；真实客服、管理端和管理端生产复核页已完成 STL 点击渲染。真实普通生产账号因当前没有分配工单，在线点击证据仍为 `PARTIAL`，未通过写正式数据强造；未实现超一期的设计软件直连，未提交、推送或部署，未修改正式业务数据；Task 8 保持 `NOT_READY`。
