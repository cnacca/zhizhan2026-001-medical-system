...
- 2026-08-18 医生端英文界面已按 D-197 完整收口并发布正式站：共享登录页保持中文，只在进入医生端后切换中文／`EN`。PR #32/#33/#35 合并 `dev`，PR #34/#36 合并 `main`，最终正式版本 `f406d105166c0b3bc6c266fead7b80c895b4d470`。自动部署 #32049802365 用时 5m38s，后端 351 项测试、release gates、不可变镜像构建／校验、发布包、备份／部署／健康检查及公网探针全部通过。英文模式覆盖 7 个主页、抽屉／对话框、下单向导、动态产品字段和隐形正畸处方；最终版线上只读复验确认中文登录页、英文医生端标签页、`A/EN Translation available`、`YYYY-MM-DD` 文本日期框和 `Orthodontic Appliance` 映射生效。患者／诊所名称、医生输入和历史消息作为原始数据保留原文；未写正式业务数据，未改权限或脱敏边界，Task 8 保持 `NOT_READY`。
