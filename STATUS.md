...
- 2026-08-16 已对《用例ID全链路回归测试》黑盒报告完成当前 `dev` 工作树复核与本地修复。报告未记录被测 Git SHA，因此未把 8 个 OPEN 项直接等同于当前源码缺陷：BUG-013 的同步互斥保存已在现有源码中，专项回归通过；BUG-011、020、025、026、027、028、029 分别补齐附件回显、医生端附件软删除入口、文件扩展名服务端白名单、四端 HttpOnly refresh cookie 会话恢复、客服 390px 响应式布局、生产端帮助入口及扫码入检精确节点/异步竞态保护。全新隔离库后端 350 tests、前端生产构建、OpenAPI、认证/退出/医生下单/部署旧缺陷/本批静态检查及 390px 真实浏览器 smoke 已通过；测试库及临时授权已删除，新专项检查已纳入正式发布门禁。该批仍是本地未提交改动，未合并、未部署或改动正式数据；文件删除继续采用可审计软删除，报告中的可见性 PARTIAL 项未在证据不足时改业务规则，Task 8 保持 `NOT_READY`。
