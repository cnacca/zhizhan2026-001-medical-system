...
- Active task: `tasks/README.md` 的「任务 8：专项验收矩阵与上线准备」，状态为 `in-progress/next-machine-readable-gap-list`
- 本轮部署安全 / 环境变量 readiness 检查第一增量已完成；新增 `npm run check:deployment-env`，并明确正式环境必须外部注入变量、默认关闭能力和禁止提交真实密钥。Task 8 总体仍保持 `NOT READY`。
...
下一轮唯一推荐目标：验收矩阵机器可读缺口清单第一增量。从 `docs/acceptance/task-8-acceptance-matrix.md` 和 `docs/deployment/task-8-final-readiness-report.md` 提炼仍未 READY 的关键项，更新 `acceptance.json` 或新增静态检查，让后续开发能通过命令发现上线缺口。Task 8 总体仍保持 `NOT_READY`，直到生产级防重放、真实弱网/跨设备续传、终检专用角色/附件、完整返工闭环、绩效完整公式/周期/申诉/标准工时配置、AI 治理闭环和部署交付材料补齐。
