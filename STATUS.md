...
- 2026-08-24 按用户确认完成客服产品目录、医生材料／色号选填、口扫批量分类上传和提示定位的本地修改，并已获授权进入正式发布流程。客服端产品目录与医生端共用已发布目录，维护入口明确归管理端“下单内容设置”；管理端“产品总览”也改为同源只读治理视图，当前发布版本显示 95 个启用产品，不再读取仅有 1 条种子数据的旧产品表。医生端主材料、制作项目、牙色系统及色号改为选填，色号支持搜索、现有选项筛选和自定义输入；色号下拉和移除确认框改为挂载在订单向导内，修复被浮层遮住而看似无响应的问题。新草稿使用 `CLASSIFIED_SCAN_BUNDLE_V3`，单次可多选 ZIP／STL／PLY／OBJ，按文件名自动推断并可手工校正上颌、下颌、咬合分类；提交须满足完整资料包或三类齐全，无法区分的散文件不算完整，PDF 不作为口扫。可修正提示点击后定位到对应步骤／产品／区域，系统错误不再提供无效修改链接。全新专用测试库后端 357 项、前端生产构建、后端 package、正式发布门禁和专项检查全部通过；隔离浏览器已验证色号候选、移除确认框、管理端 95 项目录及产品详情，控制台无错误。尚未提交、推送、部署或修改正式数据，Task 8 保持 `NOT_READY`。
