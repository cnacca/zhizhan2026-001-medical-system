...
- 2026-08-16 D-193 四端演示账号全权限修复已完成本地收口：隔离 seed 为 `admin / cs / worker / doctor` 创建各自 `ACCEPTANCE_*_FULL` 角色；生产端补齐正畸方案／批次、设备审批和成本确认入口；医生正畸响应隐藏内部生产字段；设备与成本终态必须走审批接口；RBAC 拒绝已知跨端角色家族分配。基于最新 `main` 的全新 V83 测试库上 21 个定向后端用例、四端 RBAC 静态检查、前端生产构建和 `git diff --check` 通过。发布后使用现有 RBAC 管理接口向公开演示站四账号分配角色，不直改数据库；Task 8 保持 `NOT_READY`。

- 2026-08-15 已定位正式站登录被 CORS 拒绝的具体原因：失败请求来自 `https://www.chinesedigitaldental.com`（另有直接 `:8088` 入口），而生产后端当时只允许 `https://chinesedigitaldental.com`；同一时段容器无重启，排除服务抖动。生产环境已将裸域名与 `www` 同时加入允许列表并重建后端，两个 Origin 的公网登录探针均返回预期 401 且带各自 `Access-Control-Allow-Origin`。永久修复按 D-191 将 `www` 和直接端口 308 规范化到裸域名，并在自动发布后强制验证两个登录 Origin 与两个重定向入口；本项只关闭该生产入口缺陷，不代表一期上线 readiness 完成，Task 8 保持 `NOT_READY`。
