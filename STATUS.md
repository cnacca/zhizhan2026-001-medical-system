...
- 2026-08-17 已按《线上黑盒复测》完成本地代码修复与回归：医生端“保存当前产品”改为由直接入口持有互斥锁，补上 BUG-013 漏掉的单项保存路径；生产管理员未指定人员时，绩效统计安全回落到本人，不再因 `performance:read-all` 强制缺失 `user_id` 返回 400；管理端订单与工艺生产页会根据订单内部状态跳过尚未生成工序的前置订单，不再批量制造预期 404，同时工作台补“查看全部”入口。全新隔离库迁移至 V88，后端 351 tests、前端生产构建、2026-08-16/17 专项检查、管理工作台、细分角色、四端验收 RBAC、发布链检查和管理端 Playwright 网络回归通过。用户已确认一期不包含客服代下单，客服“＋ 新建订单”只作为未来功能占位，不要求点击、跳转或调用 `POST /orders`；黑盒 `NEW-RETEST-005` 属于一期范围不匹配，应调整测试用例而不是制造空跳转。本批尚未提交、合并或部署正式网站，Task 8 保持 `NOT_READY`。
