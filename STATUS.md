...
- 2026-09-03：用户授权提交并部署后，D-217 / D-218 及检验列表文字重叠修复经 PR #58 / #59 正式发布，版本 `0eb3ffbabdd686f4ce0ecbeb436649069f10ab09`。自动部署 #33715356521 成功：完整后端 368 项、发布门禁、双镜像、备份、服务更新和公网探针通过；本地 Chrome 最新 10 项通过。未手工修改正式业务数据，未删除历史数据；Task 8 保持 `NOT_READY`。详见 `docs/deployment/order-followup-release-20260903.md`。

- 2026-09-03：D-218 已在 `codex/fix-upload-dispatch-review` 本地实施：来源快照/规则矩阵校准、产品资料分区与共享适用范围、恢复独立颌位记录/固定修复旧义齿参考、派工 409 状态按钮与生产端样式、入检/出检登记命名和权限、四端工作台下方业务跳转、生产附件预览遗漏修复。独立测试库 75 项后端回归通过；前端构建与专项检查通过。Chrome 已执行实际上传/分类/预览、混合产品和四端跳转检查；详见 `docs/acceptance/upload-dispatch-refinements-20260903.md`。未提交、推送、部署或修改正式数据；Task 8 保持 `NOT_READY`。

- 2026-09-02 按 D-217 完成订单后续需求的本地实现与关键路径真实浏览器验收：四端统一站内预览覆盖上传白名单 `STL/SLA/PLY/OBJ/PDF/JPG/JPEG/PNG/WEBP/DCM/DICOM/ZIP/DOC/DOCX/TXT`；医生新建订单改为批量投放、自动分类和待分类补录，弹性义齿移除无关侧面照，口扫完整性按完整包或上颌＋下颌＋咬合判定，产品专属资料默认折叠。四端工作台业务卡片／列表按权限跳转；完整订单号全端一致，内部工作人员端保留派生 6 位系统尾号；客服初审补全永久唯一生产单号，盒号在未发货订单中唯一、发货后释放复用。医生提交后可按阶段补充资料，说明选填，客服在已审核阶段审批并通知生产；医生只见公开节点，资料区以下默认折叠，设计确认独立于沟通。交期只保留系统预计到货日期，客服／管理员可写原因人工覆盖。生产组长使用生产账号，通过仅组长可见的“员工派工”入口在本部门候选中派工／改派，无候选时不放宽范围。发货后保留医生手工确认，并支持 7×24 小时自动确认、提前 24 小时提醒及物流异常暂停。全量后端 368 项测试、前端生产构建、OpenAPI 和专项门禁通过；真实 Edge 已验证医生制作中跳转、客服订单跳转／交期维护／STL 站内预览、生产异常跳转／组长派工入口和管理端生产异常跳转。15 类文件全组合、补充资料各阶段和 7 天时间推进仍以自动化证据为主。尚未提交、推送、部署或修改正式业务数据，Task 8 保持 `NOT_READY`。

- 2026-08-25 GOAL-037／TASK-039 已正式发布。功能经 PR #55 合并 `dev`、发布经 PR #56 合并 `main`，正式版本为 `78d68ec17f7e0c1d7f625f0594cc21c6ab07773c`。自动部署 #32843001050 按 `full` 通道用时 5m50s：后端 362 项、release gates、前后端不可变镜像构建／校验、发布包校验、MySQL 发布前备份、双容器替换、健康检查和公网登录 Origin／规范重定向全部通过；公网首页返回 200，无效登录按预期返回 401。正式版本包含医生端四步下单、设计服务数字交付与三项来源制作要求、备注选填、字段归属去重，以及螺旋扩弓器单向／双向必选规格和 V89-V95 迁移。“推簧”继续 `USER_DEFERRED`，本次未创建或修改正式订单；Task 8 保持 `NOT_READY`。

- 2026-08-25 按 D-215 完成 GOAL-037／TASK-039 本地修复。`ORTHO_SCREW_EXPANDER / 螺旋扩弓器` 保留单一产品，V95 为全部目录版本补“单向／双向”两个必选规格；医生端显示“扩弓方向 *”，未选时明确提示，服务端也拒绝绕过页面提交空规格，订单快照冻结 `variant_id / variant_code / variant_name`。V92／V93 和前后端错误推簧别名实现已移除，V95 清理开发库可能残留的别名；“推簧”继续 `USER_DEFERRED`。`ProductCatalogV2Tests` 9 项、`OrderCaseGroupTests` 12 项、目录及医生下单专项、i18n、验收基线和前端生产构建通过；本地浏览器只读确认产品仍在“扩弓类产品”且控制台无错误，未写验收订单。尚未提交、推送、部署或修改正式业务数据，Task 8 保持 `NOT_READY`。

- 2026-08-25 按 D-214 将“推簧”移出当前实施与验收范围。术语核验确认“正畸推簧／螺旋推簧”是真实专业用词，不按“弹簧”错别字归并；但本项目的产品／附件／配置身份仍未由来源定义，因此保持 `USER_DEFERRED`，不新增 SKU、别名或附件，也不再主动追问。D-211 的 V92／V93 及相关错误别名代码仍不得提交或发布，并须在任何提交前清除。TASK-039 E 当前只补“螺旋扩弓器（单向）/（双向）”选择和订单快照表达，GOAL-037 保持 `in_progress`，Task 8 保持 `NOT_READY`。

- 2026-08-25 按 D-213 重新打开 TASK-039 E 批次。《动态下单表最终版.docx》12 页内容已固化为来源快照：原表明确列有产品“弹簧矫正器”、产品“螺旋扩弓器（单向）/（双向）”和附件“加弹簧”，但没有“推簧”。测试报告只写“产品选择不够完善，例如正畸螺旋扩弓，推簧等”，不能证明“推簧”等于“弹簧矫正器”。同时确认当前目录把螺旋扩弓器单向／双向压成一个没有方向字段的通用产品。D-211 及 V92／V93 别名实现改为 `REOPENED / NOT_APPROVED_FOR_COMMIT`；尚未提交、推送、发布或修改正式数据。GOAL-037／TASK-039 恢复 `in_progress`，Task 8 保持 `NOT_READY`。

- 2026-08-25 按用户反馈完成 D-212。重新核对 SHA-256 为 `147810af568da193d7acd10f7963283ff8e0e74f5f272415aefda7bb6b1daef5` 的《动态下单表最终版.docx》后确认，设计服务“制作要求”明确包含数据格式 `STL／OBJ／EXO／3SHAPE`、设计标准“通用／个性化”和设计时间“12 小时／24 小时／3 天”；此前 D-208 取消三项字段的判断与主来源冲突，现已纠正。医生端三项恢复为必选并接入缺项定位、最终复核和旧草稿兼容；V94 为全部既有设计服务目录版本补规则，后端同时校验必填与允许值。D-210 备注选填、牙位、通用口扫、数字设计、医生确认和付款下载门禁不变。`OrderCaseGroupTests` 11 项、四步设计服务、字段归属、产品优先、最终确认、医生端 i18n、前端生产构建和差异检查通过；尚未提交、推送、发布或写正式数据，Task 8 保持 `NOT_READY`。

- 2026-08-25【历史错误口径，已由 D-213 撤销】GOAL-037／TASK-039 曾按 D-211 完成本地别名实现。源资料 SHA-256 `147810af568da193d7acd10f7963283ff8e0e74f5f272415aefda7bb6b1daef5` 只能证明“弹簧矫正器”是正式产品、“加弹簧”是正畸附件，不能证明“推簧”等于前者。V92／V93、活动目录别名返回和医生端搜索均属于未提交的错误实现，不得提交、推送或发布；当前口径以本页顶部 D-213 记录为准。

- 2026-08-25 按用户确认完成 D-210：设计服务 `case_note` 已从必填改为选填，未增加替代选项；牙位、通用口扫资料、医生设计确认和账单 `PAID` 后下载等门禁保持不变。新增 V91 追加迁移校准全部现有设计服务目录版本，目标后端 `OrderCaseGroupTests` 11 项通过并证明“缺牙位拒绝、备注为空可提交”。复核页下方“订单信息最终确认”已与上方右侧内容列统一为 260px 侧栏后的同一基线和 800px 最大宽度，窄屏保持单列。四步设计服务专项、字段归属检查、前端生产构建和差异检查通过；本地浏览器只读恢复截图对应草稿，未保存或提交。尚未提交、推送或热更新，Task 8 保持 `NOT_READY`。

- 2026-08-25 按用户反馈完成 D-209 医生制作配置字段去重。已审计固定修复、活动义齿、种植修复、常规正畸、隐形正畸和设计服务：种植系统、直径／长度、穿龈高度、连接与固位统一归“种植参数”，不再与“制作要求”重复；固定修复两个不同的边缘维度分别改为“修复体边缘形式”和“边缘位置／肩台”；页面固定控件与目录动态字段按 key 单一归属，跨规则动态字段也去重；设计服务不再渲染空的“材料与工艺”区。新增 `check:doctor-order-field-ownership`，四项医生下单专项、目录检查、前端缺陷检查、2026-08-17 回归和前端生产构建均通过。本地浏览器只读确认医生下单向导可正常打开且控制台无 error／warning；为避免写入本地患者／订单数据，没有新建草稿完成第二步写路径。尚未提交、推送或热更新；TASK-039 E 批次“推簧”目录映射仍待执行，Task 8 保持 `NOT_READY`。

- 2026-08-25 GOAL-037／TASK-039 第 1 项已完成本地实现与回归。医生下单向导由 6 步收口为“基础信息与产品、制作配置、资料上传、复核与提交”4 步，导航、缺项定位、最终确认和服务端错误映射已同步。按用户追加确认和 D-208，设计服务首步不再显示实体出货／运输，专项必填仅为牙位与设计要求备注，通用 `CLASSIFIED_SCAN_BUNDLE_V3` 资料规则保持不变；新增仅含设计确认门禁的工作流，医生确认后数字流程完成，账单 `PAID` 后才能下载原文件，预览仍可在确认前使用。前端生产构建、四步专项、既有下单检查、医生门户检查和差异检查通过；后端 `OrderCaseGroupTests`、`DesignTaskCollaborationTests`、`FileAccessTests` 共 43 项通过；本地医生端浏览器确认四步、数字设计交付文案及无实体运输字段，控制台无 error／warning，未创建或提交验收订单。尚未提交、推送或热更新；下一步仅剩 TASK-039 E 批次“推簧”目录映射，Task 8 保持 `NOT_READY`。

- 2026-08-25 用户按《2026.8.22 软件测试报告》后续清单确认分期：完整 CRM／客户关怀提醒和 STL 直接打开 CAD／设计软件转入二期；一期当前只做医生下单 6 → 4 步精简与“推簧”目录准确映射，并先做前者。已建立 D-207、GOAL-037、TASK-039 和 `docs/development/doctor-order-wizard-simplification-plan-20260825.md`，固定 4 步为“基础信息与产品、制作配置、资料上传、复核与提交”，现有字段、动态表单、正畸处方、文件规则和提交门禁一个不删、不放宽。“推簧”排在第 1 项关闭后，核对前不猜测新增或重命名产品。本轮仅整理文档，尚未修改业务代码、运行功能回归、提交、推送或部署；Task 8 保持 `NOT_READY`。

- 2026-08-25 用户明确暂不提供 AI-5 正式生产备注模板、各工序标准工时，以及正式 AI Key／预算上限／告警接收人；以上三项统一标记为 `USER_DEFERRED`，在用户主动恢复前不得继续询问、催办或作为下一步推荐。相关正式模板、绩效实值和真实 AI／告警验收仍未完成，不能据此标为 PASS；不依赖这些输入的本地开发、发布和真实文件／运维验收可继续推进，Task 8 保持 `NOT_READY`。

- 2026-08-25 用户确认此前已经提供动态表单及全部文件要求；复核仓库后确认《动态下单表最终版.docx》已有 SHA-256 指纹、完整字段矩阵和动态表单 V2 实现记录，CP-002／CP-005 均从“待客户提供”校正为 `CONFIRMED_BASELINE`。文件基线为单文件 500MB、单订单最多 50 个及类型／产品资料规则；发现的 30／50 不一致已完成本地修复：后端／测试默认、前端限制、开发／生产 env 模板、Compose 默认和 README 均统一为 50，完整发布脚本还会强制注入并复核后端容器值。`check:task9d67`、文件权限 18 项、独立测试库后端全量 358 项、前端生产构建、发布／验收门禁和 Compose 渲染均通过；共享测试库的一次搜索断言失败已由独立库复跑确认是历史重复样本污染，不是本批回归。尚未提交、推送或热更新，正式容器值仍待发布后取证；AI-5 正式模板、标准工时、真实 AI／文件／运维验收仍未完成，Task 8 保持 `NOT_READY`。

- 2026-08-25 已按 D-204 / TASK-038 完成一期验收基线校准：正式版本 `7013b1434759df6ca77a65893dbdcf0129e9af7b`、自动发布 #32742027332、HTTPS、医生端英文、D-202 站内文件预览／STL 3D、完整发布前 MySQL 备份和公网健康探针已写入当前权威证据，不再沿用 2026-08-11“未部署／无 HTTPS／i18n 约 1%／完全无备份”的旧快照。校准同时发现旧摘要存在算术错误：原 PRD V2 的 38 行实际为 29 PASS、1 PARTIAL、0 MISSING、8 EXTERNAL_ACCEPTANCE，而不是 30／1／0／7。`docs/INDEX.md`、`docs/DELIVERY-GAP.md`、`acceptance.json`、9D.81 和机器检查已对齐；九类 readiness 仍全部 `PARTIAL`，数据库／MinIO 恢复、监控／日志／回滚、真实 AI、真实文件网络、双实例 WebSocket、普通生产账号 STL 点击、客户输入、培训和总体验收均未伪造完成。本批不改业务代码、不写正式数据、不触发热更新；Task 8 保持 `NOT_READY`。

- 2026-08-24 D-203 正式发布提速已启用：修复经 PR #52 合并 `dev`、PR #53 合并 `main`，正式版本 `7013b1434759df6ca77a65893dbdcf0129e9af7b`。`Deploy production` 自动比较 main merge commit 与第一父提交；只有 `frontend/**` 加受控发布文档时走 `frontend` 快速通道，其他改动和手工发布全部 fail closed 为 `full`。首次发布因包含工作流／脚本而按预期走 `full`，自动部署 #32742027332 用时 10m21s，后端 358 项、release gates、双镜像构建／校验、MySQL 备份、双容器替换、健康检查和公网探针全部通过；部署后首页和后端健康接口均返回 200。纯前端通道会跳过后端测试基础设施、后端全量测试／打包／镜像、MySQL 备份和后端重启，只上传并替换经校验的前端镜像；PR 校验、前端回归门禁、SHA-256、known_hosts、部署锁、前端回滚标签、双健康检查和公网探针继续保留。首个真实纯前端发布耗时尚未产生，不把 2–3 分钟估算写成实测；未修改正式业务数据，Task 8 保持 `NOT_READY`。

- 2026-08-24 D-202 已发布正式站：客服“退回补资料”闭环、待生产审核附件只读权限、客服全局 STL 查看器、管理端站内文件预览，以及 MinIO 预览／下载 `inline`／`attachment` 分流均已上线；管理端预览不再打开空白页或自动下载。修复经 PR #49 合并 `dev`、PR #50 合并 `main`，正式版本 `e19bce4d70e672920d6d3c5430f9743946e96d4a`；自动部署 #32731578654 用时 4m57s，完整后端 358 项、release gates（含校准后的 `check:feedback-20260713` 和新增 `check:feedback-20260822-blockers`）、不可变镜像构建／校验、发布包、上传、备份／部署／健康检查及公网登录 Origin／规范重定向全部通过。部署后正式站首页返回 200、后端健康为 `ok`，线上前端产物已确认包含“浏览器 3D 查看”新链路；隔离浏览器 smoke 为 3/3，已断言 STL 实际解析并生成 canvas、预览接口只调用一次、下载接口不调用且不新增标签页。真实客服、管理端和管理端生产复核页已有 STL 点击渲染证据；真实普通生产账号因当前没有分配工单，在线点击证据仍为 `PARTIAL`，未通过写正式数据强造。未实现超一期的设计软件直连，未修改正式业务数据；Task 8 保持 `NOT_READY`。

- 2026-08-24 D-201 已发布正式站：客服产品目录和管理端产品总览统一读取医生端当前已发布目录，新增／修改入口归管理端“下单内容设置”；医生端材料／色号选填、色号搜索与自定义、口扫 ZIP／STL／PLY／OBJ 批量分类上传、精确提示定位，以及被遮挡的色号候选层和移除确认层均已修复。修复经 PR #46 合并 `dev`、PR #47 合并 `main`，正式版本 `01919fb61c6582d2e8d3e124ceddcce3b613749f`；自动部署 #32698278453 用时 5m37s，完整后端 357 项、release gates、不可变镜像构建／校验、发布包、上传、备份／部署／健康检查和公网登录 Origin／规范重定向全部通过。正式站首页返回 200、后端健康为 `ok`；只读浏览器确认管理端“产品总览”显示当前发布目录 95 个产品，并明确新增或修改进入“下单内容设置”。本次未创建订单或修改正式业务数据，Task 8 保持 `NOT_READY`。

- 2026-08-21 医生多产品下单、共享／专属资料、缺项提示、返回上一步草稿保留、提交校验、时间显示、色号与材料同步已按用户要求发布正式站。业务改动经 PR #41 合并 `dev`、PR #42 合并 `main`；首次自动部署 #32472509316 的完整后端测试通过，但旧发布静态检查仍匹配已替换的 `currentStepErrors`，在构建镜像前安全阻断，未接触正式服务器。门禁校准经 PR #43 合并 `dev`、PR #44 合并 `main`，最终正式版本 `bf49ca3fdde9d62834504a29a2f79e5ec3de38dc`；自动部署 #32472847396 用时 5m54s，完整后端测试、release gates、不可变镜像构建／校验、发布包、上传、备份／部署／健康检查及公网登录 Origin／规范重定向全部通过。公网首页和后端健康接口均返回 200，线上医生端产物已核对包含本批关键新文案；未写正式业务数据，Task 8 保持 `NOT_READY`。

- 2026-08-21 按用户追加需求完成医生下单缺项反馈：点击“下一步”时会在当前页顶部持续列出步骤、产品和具体缺失字段／资料，并可通过“去填写”进入对应步骤和产品；资料缺项明确区分上颌、下颌、咬合扫描以及共享／专属补齐方式。最终提交按钮不再因缺项静默禁用，点击后会列出全部未完成项，但前后端提交校验不放宽。本地前端生产构建、专项检查、医生端 i18n、产品优先检查、acceptance 和差异检查通过；隔离浏览器已验证第一步 3 项缺失、产品咬合缺失、定位回填、最终两项确认缺失和正常布局，控制台无错误，未提交测试订单。未提交、推送或部署，Task 8 保持 `NOT_READY`。

- 2026-08-21 按用户追加确认扩展 GOAL-036 / TASK-037：病例共享区和每个产品专属区统一提供上颌、下颌、咬合、比色照片、口内咬合照、旧义齿参考六类资料。每个产品的前三类必传可由共享资料继承或由专属文件满足；上传同类专属文件后仅当前产品优先使用，并明确显示继承的共享文件名。新草稿使用 `FIXED_LAYERED_V2`，历史 `FIXED_SHARED_V1` 保持兼容；前端生产构建、专项检查和全新隔离数据库 `OrderCaseGroupTests` 6/6 通过。本地隔离浏览器已实传 3 个共享 STL，并为其中一个产品实传 1 个专属 STL：两个产品的继承状态、单产品覆盖状态及最终页“共享 3 个／专属 0 或 1 个”摘要均正确，控制台无错误；未点击最终提交。未提交、推送或部署，未修改正式数据，Task 8 保持 `NOT_READY`。

- 2026-08-18 GOAL-036 / TASK-037 医生下单最终确认与共享资料上传已完成本地开发收口。上颌、下颌、咬合扫描固定为病例共享必传，其他资料保持选传，固定槽位支持拖拽与点选；最终页按产品展示产品、数量、制作/默认/附加要求、附件数量、制作周期和价格占位；询问客服复用现有订单消息且不阻止提交。专项检查、前端生产构建、后端编译、隔离库 `OrderCaseGroupTests` 5/5 和 acceptance 通过；真实 Chrome 已发起上传，但本地 MinIO bucket 在 Multipart initiate 返回 503，浏览器全路径记为 `PARTIAL`。未提交、未推送、未部署，价格仍显示“价格待配置”，Task 8 保持 `NOT_READY`。

- 2026-08-18 医生端英文界面已按 D-197 完整收口并发布正式站：共享登录页保持中文，只在进入医生端后切换中文／`EN`。动态系统文案补漏经 PR #38 合并 `dev`、PR #39 合并 `main`，最终正式版本 `f64b951155926df48b5c20afa58ae5e4a2a77212`。自动部署 #32056401585 用时 5m41s，后端 351 项测试、release gates、不可变镜像构建／校验、发布包、备份／部署／健康检查及公网探针全部通过。正式站按反馈截图只读复验确认公开进度 8 个节点、草稿说明、公开进度空提示、订单时间线、通知标题／系统摘要及物流“医生已确认收货”事件均为英文；中文登录页继续保留。患者／诊所名称、上传文件名、承运商名、医生输入和消息正文作为原始业务数据保留原文；未写正式业务数据，未改权限或脱敏边界，Task 8 保持 `NOT_READY`。

- 2026-08-17 《线上黑盒复测》修复已通过 PR #24 合并 `dev`、PR #25 合并 `main`，正式发布版本为 `f2c5d6a73f81741db267a0ebf50ea72963203beb`。医生端“保存当前产品”由直接入口持有互斥锁，补上 BUG-013 漏掉的单项保存路径；生产管理员未指定人员时，绩效统计安全回落到本人；管理端前置订单不再请求尚不存在的工序实例，工作台补“查看全部”。自动发布运行 #32001893440 在隔离库通过后端 351 tests、2026-08-17 专项 release gate、不可变镜像构建/校验、发布包校验、生产备份/部署/健康检查及公网登录 Origin/规范入口探针，结果为 `success`。正式站首页返回 200，线上 JS 包已核对包含 `admin-view-all-todos`、“查看全部”及前置状态判断，网站自身控制台无 error/warning；本轮未登录写生产数据，页面内交互继续以本地 Playwright 网络回归为证据。用户已确认一期不包含客服代下单，`NEW-RETEST-005` 属于一期范围不匹配并已由 D-196 固定，测试方仍需按该正式 SHA 重跑其余黑盒用例。Task 8 保持 `NOT_READY`。

- 2026-08-16 《用例ID全链路回归测试》缺陷批次已通过 PR #21 合并 `dev`、PR #22 合并 `main`，正式发布版本为 `54eea0e4399782485d7953231e4768ed684cd097`。自动发布运行 #31939878141 完成后端全量测试、release gates（含本批专项检查）、不可变镜像构建/校验、发布包上传、生产备份/部署/健康检查及公网登录 Origin/规范入口探针，结果为 `success`。正式站只读浏览器复验确认 DOCTOR／CS／PRODUCTION／ADMIN 四端刷新后均保持登录；客服 390px 下文档宽度 390px、顶部工具区位于 10–380px，无横向溢出；生产帮助入口/抽屉和扫码登记页节点编号输入可见，网站自身控制台无 error/warning。正式生产验收账号当时没有可执行“我的任务”，医生账号没有可恢复附件草稿，因此未在正式数据上制造附件删除、非法上传或质检提交；对应写路径仍以隔离库 350 tests、本地浏览器/静态回归为证据。所有浏览器测试账号已退出，未创建订单或修改业务数据；文件删除继续采用可审计软删除，可见性 PARTIAL 项未在证据不足时改业务规则，Task 8 保持 `NOT_READY`。

- 2026-08-16 D-192／D-193 确认边界已通过 PR #19 与 CI 隔离修复 PR #20 合并 `main`，正式发布版本为 `e465d8d8d13b30c2e6d265fea3e3f82e55b2e9ec`。首次自动发布运行 #31936990708 在镜像构建前被 Linux 测试顺序污染安全阻断，未接触服务器；修复夹具 ID 冲突与迁移断言范围后，随机类顺序 348 tests 全绿。第二次运行 #31937188201 完成后端测试、release gates、不可变镜像构建与校验、发布包上传、生产部署、容器健康、公网登录 Origin 和规范重定向检查，结果为 `success`。正式站真实浏览器只读验证 DOCTOR／CS／PRODUCTION／ADMIN 四个测试账号均返回 200 并进入对应门户，未创建或修改订单；现有四端 smoke 的旧登录选择器已同步校准。该批次上线不改变一期 Task 8 `NOT_READY`。

- 2026-08-16 D-192／D-193 确认边界已合并并推送 `dev`：四端验收账号保持各自端全功能，正式账号继续由细分角色、专项权限和 DataScope 约束；设计技术内审归生产侧获授权组长。与 `main` 对账时只保留已确认的正式站 CORS／规范入口修复，物流、QC、正畸批次／设备审批／成本确认端口归属、RBAC 角色家族及推测性导航映射等未确认改动未带回。全新隔离库迁移至 V88，后端 348 tests 全绿；前端生产构建、四端验收 RBAC、前端缺陷审计、认证刷新／退出竞态、OpenAPI、部署环境、自动发布、Compose、acceptance 与差异检查全部通过。未改正式数据、未重播现有 demo、未合并 `main`、未触发生产部署，Task 8 保持 `NOT_READY`。

- 2026-08-15 D-192 技术缺陷修复保留：细分角色不再被入口角色宽权限架空，Refresh Token 并发轮换／退出、停用账号 WebSocket、医生写权限、Multipart 完成校验、STL 并发绑定、前端 Mock／跨账号敏感状态及发布门禁等不依赖新增业务口径的问题继续修复。物流与工作流业务语义改动已从本批撤回，原先基于它们的全量测试数量不再作为当前验证结论；尚未提交、部署或修改正式数据，Task 8 保持 `NOT_READY`。

- 2026-08-15 已按 D-191 固定正式网站“热更新”约定：后续统一走“修改与必要验证 → 按分支规则通过 PR 合并 `main` → 现有 `Deploy production` 自动完整部署 → 最小线上检查”。不另建前端专用发布链路，不把开发环境 HMR、直接替换服务器静态文件或只更新前端容器作为默认方案；除非用户明确要求优化部署时长，否则不扩大为部署体系改造。现有发布门禁、备份、回滚和安全检查保持不变。
