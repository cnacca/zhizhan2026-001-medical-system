...
- 2026-08-14 正式站点客服新增客户 403 热修复已部署：线上 `CS -> clinic:create` 权限记录由 0 增至 1，权限表已备份至 `/home/ubuntu/deployment-backups/20260814-153643-cs-hotfix/system_role_permission.sql`；前端使用独立分支 `fix/cs-clinic-create-permission@24cfd5dc` 的已验证构建产物，只向真实持有 `clinic:create` 的账号显示新增按钮。当前正式前端镜像 revision 为 `24cfd5dc7b91004b3c2713d74e2775be4186eb84`，旧镜像保留为 `ai-order-platform-frontend:rollback-before-cs-24cfd5dc`。公网首页加载 `index-Dq0JlSMO.js`，后端健康、正式 CORS 与五个一期容器状态正常；临时 CS Bearer 对空请求体调用 `POST /clinics` 已从权限层进入参数校验并返回 400 而非 403，未创建客户数据。浏览器刷新会清除当前登录会话，客服仍需重新登录后完成真实建档复测；Task 8 保持 `NOT_READY`。

- 2026-08-14 已将医生订单删除／取消功能和当前部署状态整理为新窗口交接：实现继续停留在 `fix/doctor-order-delete-cancel` 的未提交本地工作树，后续开发按 `docs/development/doctor-order-delete-cancel-handoff-20260814.md` 和 D-187 接手。当前已完成草稿单删／最多 50 条原子批量软删除、附件停用与审计、审核中订单幂等取消申请、V84、OpenAPI 和自动化验证；尚未实现多产品病例草稿一致交互、客服／管理员同意或拒绝、真正“已取消”状态、通知、撤回／重提和审批后的设计／生产／账单／退款／物流规则。本轮按用户要求只回写文档，以上闭环延期到后续窗口；本分支功能仍未发布到正式站点。标准本地运行脚本当前只确认 MySQL／Redis／MinIO，backend/frontend 需接手时重新核对；演示库另有 V77 checksum mismatch，未经核对和备份不得 repair/reset。正式域名已完成三域名 HTTPS、续期 dry-run、正式 CORS／文件公网端点和后端重启，但真实四端业务复测、8088/8080/22 防火墙收口、恢复演练和本功能发布仍未完成。Task 8 保持 `NOT_READY`。

- 2026-08-14 医生端订单删除／取消入口已在 `fix/doctor-order-delete-cancel` 完成本地开发：草稿行提供“继续编辑／删除草稿”，草稿筛选提供最多 50 条的原子批量软删除；已提交且仍处于客服或生产待审核的订单在“更多”中提供“申请取消”，只创建待处理申请，不直接删除或变更订单状态。V84 保留订单与审计，删除草稿的 ACTIVE 附件同步转为 `DELETED` 并留文件审计；患者历史、订单列表和病例组读取过滤已删除草稿。全新隔离库从 V1→V84 迁移成功，29 项订单／患者／病例组组合回归与补充后的 24 项订单专项测试均 0 失败，前端生产构建和 OpenAPI 197 paths / 226 operations 校验通过。**该修复尚未发布到正式站点，未操作任何线上订单**；正式发布后仍需用真实医生账号复测单删、批删、取消申请和权限边界。当前状态机没有“已取消”状态，申请的后台审批和批准后的业务处理仍待规则确认，Task 8 保持 `NOT_READY`。

- 2026-08-13 正式域名部署已推进到 `DNS_READY / HTTP_ROUTING_READY`：GoDaddy 停放页记录已删除，Google Public DNS 与 Cloudflare DNS 均确认 `chinesedigitaldental.com`、`files.chinesedigitaldental.com` 只解析到 `43.129.232.106`，`www` CNAME 指向主域名；宿主 Nginx 已分别代理系统至 `127.0.0.1:8088`、文件至正式 MinIO 的 `127.0.0.1:19000`，两个公网 HTTP 健康请求均返回 200。正式 MinIO 继续使用原数据卷，私有仓库只读 deploy key 和干净代码副本 `/home/ubuntu/releases/ai-order-2fea96af` 已准备，但业务容器未切换版本。当前仍缺 HTTPS 证书、正式 CORS/MinIO 公网端点、后端重启、真实浏览器业务复测、`3306/6379/8088/9000/9001` 公网端口收紧及恢复演练；医生产品点击故障仅能写为“预计由 HTTPS 安全上下文解除”，尚未验证。Task 8 保持 `NOT_READY`，实时记录见 `docs/deployment/production-domain-deployment-progress-20260813.md`。

- 2026-08-13 **较早部署快照（已被上方最新状态取代）**：进入 `chinesedigitaldental.com` 正式域名部署准备；当时已核实腾讯云香港 `43.129.232.106` 上的 8088 Compose、运行容器、数据卷、宿主 Nginx 和服务器脏工作区，并完成 MySQL、MinIO、Redis、`.env`、Git 仓库及未提交改动的首次非停机备份，目录为 `/home/ubuntu/deployment-backups/20260812-230011`。该快照记录时 DNS、Nginx 域名站点和 HTTPS 尚未执行；当前事实以上一条及 `docs/deployment/production-domain-deployment-progress-20260813.md` 为准。
...
- 本轮 9D.98 AI-5 生产备注客户模板 / 知识上下文补强第一增量已完成：`/ai/production-note` 响应新增 `template_version=PHASE_ONE_DEFAULT_V1`、`knowledge_context_notes` 和 `requires_customer_template_confirmation`，默认模板明确标注客户模板未确认；新增 `/ai/production-note/confirm` 人工确认写入链路，CS / WORKER / ADMIN 在订单数据范围内确认后才追加写入 `orders.production_note` 并写 AI 审计。客服初审页新增 AI-5 生产备注草稿、知识上下文说明和人工确认写入入口；`npm run check:task9d98` 同时复核 AI-1 `/ai/translate`、AI-4 `/ai/check-missing`、AI-2 `/ai/cs-query` 与 AI-5 入口。本轮不新增迁移，不接真实 DeepSeek key，不伪装真实客户模板或客户签字，不自动下发生产指令。Task 8 仍保持 `NOT_READY`。
