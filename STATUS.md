...
- 2026-08-17 已按 D-197 完成医生端中英文界面的本地收口：英文模式现覆盖 7 个主页、通知／详情／编辑对话框、六步下单向导、动态产品字段和隐形正畸七步处方，切换选择写入浏览器本地设置。系统状态和已知目录值使用受控英文映射；患者姓名、诊所名称、医生输入和历史消息继续显示原文。`check:doctor-portal-i18n`、原医生端／产品优先下单专项检查、前端生产构建和差异检查通过；未改后端、数据、权限或脱敏边界，尚未合并或部署，Task 8 保持 `NOT_READY`。
