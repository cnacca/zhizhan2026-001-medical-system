...
- 2026-08-16 已按用户要求收紧 D-193：只保留明确确认的四端验收账号、基础角色／细分角色聚合规则，以及“生产侧获授权组长执行设计技术内审，客服只查看医生可见版本”的权限调整。物流共享队列与状态拆分、QC 自动投影和必检跳过、正畸批次／设备审批／成本确认的端口归属、RBAC 角色家族限制及大范围前端导航权限映射均因未确认而撤回。全新隔离测试库迁移至 V88，角色／设计／正畸定向 27 tests 全绿；前端生产构建、OpenAPI、验收 RBAC、前端缺陷审计和 `git diff --check` 通过。默认测试库仍有既有 V73 checksum 漂移，未 repair；未改正式数据、未重播 demo、未提交或部署，Task 8 保持 `NOT_READY`。
