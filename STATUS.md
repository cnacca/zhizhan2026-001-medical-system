...
- Active task: `tasks/README.md` 的「任务 2：数据库模型与 9 条工序链初始化」，状态为 `completed-with-db-and-http-smoke`
...
项目处于 M1/M2 交界准备阶段：接口契约已冻结，TRD V1.1 的技术口径已落入开发计划，任务 1 项目骨架已完成，任务 2 数据库基线与 9 条工序链初始化已完成数据库和 HTTP 烟测。下一步进入任务 3：订单状态投影与医生端脱敏基础。
...
- 后续实现订单状态投影、医生端脱敏 VO、医生端安全读模型和 AI-3 上下文边界。
- 后续实现订单生产审核后的工序实例化、派工、工时、入检/出检、返工；任务 2 只完成表结构、种子数据和只读链查询。
- 后续把 Redis、MinIO 接入业务模块；当前任务只完成 MySQL/Flyway 接入。
...
进入 `tasks/README.md` 的「任务 3：订单状态投影与医生端脱敏基础」。开始前应先锁定 `internal_status` / `external_status` 枚举、`OrderStatusProjector` 写入边界、`OrderDoctorVO` 与 `DoctorOrderAssistantReadModel` 字段白名单，继续保证医生端接口、医生端 WebSocket、医生端文件访问和 AI-3 不暴露内部工序/员工/质检/工时/返工/绩效字段。
