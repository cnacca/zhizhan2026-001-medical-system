...
状态：用户已于 2026-08-17 明确要求医生端英文界面，并于 2026-08-18 确认共享登录页不做英文版。最终收口经 PR #32/#33/#35 合并 `dev`、PR #34/#36 合并 `main`，正式发布版本为 `f406d105166c0b3bc6c266fead7b80c895b4d470`。自动部署 #32049802365 结果为 `success`。
...
- 患者姓名、诊所名称、医生录入的备注／医嘱、历史消息等原始业务数据保持原文，不伪造或机器翻译医疗内容。系统状态、产品类型和已知目录选项使用受控英文映射；未配置权威英文名称的目录项使用稳定 code 的可读回退，不自行发明业务译名。
- 本次只改医生端展示层，不改后端接口、数据库、RBAC、DataScope或医生端脱敏边界。
- 新增 `check:doctor-portal-i18n` 防止已覆盖的英文界面回归为中文；原医生端和产品优先下单检查继续保留。

影响：`docs/acceptance/phase-two-scope-baseline-20260726.md` 中“多语言不在范围内”保留为当时基线，但被本决策对“登录后的医生端界面”局部取代。客服、生产、管理三端，共享登录页，英文操作手册、海外时区和海外业务规则仍不在本次范围。正式站只读浏览器已遍历 7 个医生端主页和下单子流程，最终版复验了中文登录页、英文标签页、消息翻译标记、日期控件和正畸产品英文映射；未写正式业务数据。Task 8 继续保持 `NOT_READY`。
