...
状态：用户于 2026-08-24 指出一次“热更新”端到端耗时 31 分 29 秒，并明确要求优化；已在修复分支完成本地实现与静态验收，尚未合并或部署。
...
影响：纯前端发布包不再携带后端 JRE／应用镜像，上传和容器替换范围明显缩小；涉及 D-203 自身工作流／脚本的首次发布必然自动走 `full`。本地开发仍按改动范围运行必要验证，发布文档回写不再阻塞用户看到上线结果。该优化不改变 Task 8 `NOT_READY`。
