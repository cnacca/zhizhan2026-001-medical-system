...
## D-193 四个公开演示账号按端口授予完整验收能力

状态：用户已确认；代码、隔离 seed、权限边界和定向验证已完成，按项目发布约定通过 PR 发布。

决策：

- `admin / cs / worker / doctor` 是四端演示验收账号，不作为真实岗位最小权限模板。四个账号分别使用 `ACCEPTANCE_ADMIN_FULL / ACCEPTANCE_CS_FULL / ACCEPTANCE_PRODUCTION_FULL / ACCEPTANCE_DOCTOR_FULL` 聚合所属端现有权限；管理、客服、生产范围为 `ALL`，医生范围为 `CLINIC`。
- 验收角色不进入 Flyway。隔离环境由受 `DEMO_ISOLATED_ENV=true` 与 `_demo` 数据库名双门禁保护的 seed 创建；公开演示站发布后通过现有 RBAC 管理接口创建、授权和分配，保留审计记录，不直接修改数据库。
- 医生端即使拥有本端完整验收能力，也不得返回内审、生产批次、员工标识、工时、返工、绩效等内部数据。设备维修／报废申请和成本记录不得在创建时直接进入审批终态。
- 设计内审、正畸生产批次、设备审批、成本确认和终检报告采用“专用权限码 + 允许的端口”双重校验；生产端补齐对应操作入口，RBAC 按账号 `user_type` 拒绝已知跨端角色家族分配。

影响：真实岗位角色继续按 V79/V80 最小授权；本决策只扩大四个明确演示账号的端内验收覆盖，不改变 Task 8 `NOT_READY`。
...
影响：医生下单界面恢复“产品优先”流程，同时保留后端病例订单组必须绑定 `patient_id` 的数据约束；不新增数据库字段或接口，不修改历史订单。新增 `check:doctor-order-product-first`，本地浏览器只读验证暂选、取消与选择患者后状态保留；正式网站尚未部署，Task 8 保持 `NOT_READY`。
