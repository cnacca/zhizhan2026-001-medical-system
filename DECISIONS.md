...
状态：用户已确认验收账号与真实岗位的总体口径；只落实已有明确文件支持的权限归属，未确认的跨端功能归属保持不变，尚未合并、部署或写入现有 demo 环境。
...
状态：已保留用户授权修复且不依赖新增业务口径的技术缺陷；涉及业务角色、物流和状态机语义的未确认改动已撤回，尚未合并或发布。
...
影响：新增 V86 Refresh Token family，权限、文件、前端会话、Compose 环境变量及发布门禁同步更新。物流共享范围、物流状态拆分和工作流 QC 投影不在本决策内。本轮只完成本地代码与自动化验证，不执行 Flyway repair、不修改正式数据、不提交 PR 或部署正式环境；Task 8 保持 `NOT_READY`。
