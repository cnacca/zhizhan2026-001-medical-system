...
状态：用户已于 2026-08-17 明确要求医生端英文界面；实现经 PR #27 合并 `dev`、PR #28 合并 `main`，正式发布版本为 `78c0955b35f519f77bf28b107e8ae5a1b138cfd6`。自动部署运行 #32033871635 结果为 `success`。
...
影响：`docs/acceptance/phase-two-scope-baseline-20260726.md` 中“多语言不在范围内”保留为当时基线，但被本决策对“医生端界面”的范围局部取代。客服、生产、管理三端，英文操作手册、海外时区和海外业务规则仍不在本次范围。正式站首页返回 200，线上实际医生端 JS/CSS 资源已核对包含英文页面、下单向导、正畸处方和语言持久化标记；当前浏览器读取通道超时，未把登录后真实点击记为已完成。Task 8 继续保持 `NOT_READY`。
