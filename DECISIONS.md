...
状态：用户已于 2026-08-24 确认先处理阻断项并随后要求校准、补回归；本地实现与自动化验收已完成，未提交、推送或部署。
...
影响：新增专项静态检查和隔离浏览器 smoke，并补真实客服／管理端／管理端生产复核页 STL 点击回归与签名响应头测试；旧 2026-07-13 门禁改为要求站内查看器并禁止 `about:blank`／`popup.location.replace`，普通 `WORKER`＋`SELF` 生产看板隔离浏览器回归断言 canvas 渲染、只请求预览、不请求下载且不新增页面。真实普通生产账号当前没有分配工单，因此在线点击证据仍为 `PARTIAL`，不通过写正式数据强造；文件对象、MinIO 私有桶、短时签名 URL、既有 DataScope、设计稿可见性和审计规则保持不变。本地阻断修复通过不等于正式环境复测或一期整体交付完成，Task 8 继续保持 `NOT_READY`。
