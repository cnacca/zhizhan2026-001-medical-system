...
## D-188 客服创建客户继续使用 clinic:create 权限码并修正授权种子

状态：`ACCEPTED`

- D-136 已确认 CS / ADMIN 均可创建基础诊所档案，因此正式站点客服建档 403 是实现偏差，不是需要重新扩大的业务范围。
- `clinic:create` 继续作为唯一服务端门禁，不以“从客服端登录”直接放行；V85 把该权限增量授予 CS 入口角色，已有自定义角色仍需显式持有权限码。
- Controller 和 Service 层继续以 `clinic:create` 为真实门禁，不新增 CS 入口角色直通；这样本地兼容模式和正式严格权限模式不会形成不同的业务授权结果。
- 客服端新增按钮按登录响应中的真实权限码显示。没有 `clinic:create` 的账号不显示入口，避免出现“能填表但提交必然 403”的假操作。
- 权限在 access token 中快照保存；数据库迁移与后端部署完成后，已登录客服需要刷新 token 或重新登录才会获得新权限。

影响：本决定修复 V36 权限种子、后端注解和 2026-07-20 客户管理页面之间的不一致，不改变医生、生产人员或客户数据范围，不代表正式站点已经发布或通过真实客服账号验收；Task 8 保持 `NOT_READY`。

## D-187 取消订单完整闭环延期实现并以独立交接清单约束

状态：`ACCEPTED`

- 当前 `fix/doctor-order-delete-cancel` 只交付医生草稿软删除和审核中订单“提交取消申请”的本地第一段，不把 `PENDING` 申请解释为订单已经取消。
- 客服／管理员的取消申请列表、同意／拒绝、医生撤回、取消结果通知、正式“已取消”状态及报价／设计／生产／账单／退款／物流后果全部延期到后续独立开发窗口。
- 后续实现前必须先确认允许取消的阶段、处理角色、审批后下游规则、撤回／重提／恢复规则；未确认前不得自动退款、删除任务、删除文件或直接修改生产数据。
- 多产品病例草稿不得继续表现为普通单订单删除：后续优先采用“单产品可直接删除；多产品进入病例编辑器并整组管理”的交互，或在业务明确允许拆子订单后再设计原子删除规则。
- 后续必须补客服工作台、审批 API、状态机、外部状态投影、通知、审计、幂等／并发／权限／多产品病例测试，并在正式发布前完成真实医生与客服账号验收。
- 当前未提交工作树、验证结果、环境风险和接手顺序统一记录在 `docs/development/doctor-order-delete-cancel-handoff-20260814.md`。后续窗口先读该文件，不得重做已经通过的本地实现，也不得把它写成正式站点已发布。

影响：本决定只冻结后续边界和交接顺序，不新增本轮代码。D-186 的本地第一段继续有效；取消闭环、正式发布和 Task 8 readiness 均未完成，Task 8 保持 `NOT_READY`。

## D-186 医生订单按生命周期提供草稿软删除和审核中取消申请

状态：`ACCEPTED`

- 草稿订单在列表“操作”列提供“继续编辑”和“删除草稿”；删除采用软删除，订单主记录与审计信息保留，医生列表、患者历史和病例组读取不再返回该草稿。
- 删除草稿时，该草稿的 ACTIVE 附件同步标记为 `DELETED` 并写入文件访问审计；不物理删除对象、历史订单、工时、返工或设计稿版本。
- 批量删除只在医生端“草稿”筛选下出现，最多 50 条，只接受当前医生、当前诊所、仍为单产品草稿的订单；任一项不满足时整批回滚。多产品病例草稿继续从病例编辑器统一管理，避免只删除一件产品破坏病例组。
- 已提交且仍处于客服或生产待审核状态的订单，在“更多”中提供“申请取消”；请求必须填写原因，只创建可审计的待处理申请，不删除订单，也不自动改变订单状态。重复请求返回同一条待处理申请。
- 不提供“批量申请取消”。已进入生产、发货、完成或其他非审核状态的订单不开放取消申请入口。

影响：新增 V84、医生订单生命周期 API、Doctor Portal V2 操作入口与 OpenAPI 契约。现有订单状态机没有“已取消”状态，客服／管理员批准或拒绝申请、批准后的设计／生产／账单处理规则不在本次按钮范围内，后续必须先确认业务规则再实现。本决定只代表本地代码与隔离测试完成；正式站点在该修复分支发布并完成真实医生账号复测前，不能写成线上已可用，Task 8 继续保持 `NOT_READY`。
