...
- 一期付款状态先采用人工维护口径，由 CS / ADMIN 维护对外付款状态，不接真实支付系统。
...
- 付款状态不代表真实支付流水、财务审批或支付渠道对账。
- 若客户 / PM 修改付款状态口径，需要先更新确认清单和本决策记录，再继续实现。
...
- Task 8 仍保持 `in-progress / NOT_READY`；下一轮继续按预授权队列收束下一个最小闭环。
